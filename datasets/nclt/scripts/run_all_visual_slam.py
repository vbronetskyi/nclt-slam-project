#!/usr/bin/env python3
"""
run_all_visual_slam.py - Overnight Visual SLAM Benchmark on NCLT Dataset
========================================================================

Runs DROID-SLAM, DPVO, and DPV-SLAM (DPVO + loop closure) on NCLT spring
and summer sessions. Fully autonomous — no human input required.

Methods:
  - DROID-SLAM  (Teed & Deng, NeurIPS 2021)
  - DPVO        (Teed et al., 2024) — visual odometry, no loop closure
  - DPV-SLAM    (Lipson et al., ECCV 2024) — DPVO + learned loop closure

Camera: Ladybug3 Cam0, f=221, cx=404, cy=308 at 808x616 half-res
Sessions: 2012-04-29 (spring), 2012-08-04 (summer)

Usage:
    python run_all_visual_slam.py

    # or in tmux for overnight:
    tmux new -s slam
    python /workspace/datasets/nclt/scripts/run_all_visual_slam.py 2>&1 | tee /workspace/datasets/nclt/results/week0_visual_slam/run.log
"""
...
# camera intrinsics (COLMAP-calibrated, half-res 808x616)FX, FY = 221.0, 221.0
CX, CY = 404.0, 308.0
IMG_W, IMG_H = 808, 616

# reference LiDAR ICP results from Experiment 0.1LIDAR_ATE = {"2012-04-29": 174.0, "2012-08-04": 188.2}
