...
# use the most complete GTgt = gt_seasonal if gt_seasonal is not None else gt_icp_dense
...
# --- ORB-SLAM3 mono-inertial (B001 — best VIO run, massive drift) ---
...
# panel 1: LiDAR methods + GTax = axes[0, 0]
...
# panel 2: Visual SLAM methods + GTax = axes[0, 1]
...
# panel 3: ORB-SLAM3 methods + GTax = axes[1, 0]
...
# panel 4: Best of each categoryax = axes[1, 1]
...
# pick best from each categorybest_picks = {}
for name, data in trajs.items():
    cat = data['method']
    if 'ORB' in name:
        cat = 'orbslam'
...
# tracking ratetracking_data_sorted = sorted(tracking_data, key=lambda x: x[1], reverse=True)
...
# number of poses (log scale)n_poses_data = sorted(tracking_data, key=lambda x: x[2], reverse=True)
...
# left: VIO trajectory (raw, showing catastrophic drift)
if orb_b001 is not None:
    ax = axes[0]
...
# right: Mono trajectory (no IMU) — stays small but no scaleorb_v2_full = load_orbslam3(RESULTS / 'week0_orbslam3_v2/trajectory_spring_full.txt')
...
summary_rows = [
    ...
    ['ORB-SLAM3 VIO', 'Cam5 + MS25 IMU', 'Spring', '>10000', '90.2%', 'CATASTROPHIC scale drift'],
...
for i in range(1, len(summary_rows)):
    ...
    for j in range(len(row)):
        ...
        elif 'CATASTROPHIC' in row[5] or 'FAILED' in row[5]:
...
footnote = (
    ...
    "• ORB-SLAM3 VIO completely fails — 47Hz MEMS IMU + 5fps camera insufficient for scale estimation\n"
