#!/usr/bin/env python3
"""
ICP Odometry + Loop Closure + Pose Graph Optimization (v3 - Fixed)
==================================================================
Fixes:
- Scan context ONLY for candidate detection (no spatial proximity from drifted traj)
- FPFH + RANSAC global registration for verification (handles arbitrary poses)
- Aggressive deduplication: 1 closure per 50-frame window pair
- Levenberg-Marquardt damping in pose graph to prevent divergence
"""
import os
import sys
import numpy as np
...
# scan contextSC_SECTORS = 60
SC_RINGS = 20
SC_MAX_RANGE = 80.0

# loop closureLC_MIN_GAP = 200
LC_SC_THRESH = 0.35        # Relaxed SC threshold (rotation-invariant)
LC_CHECK_INTERVAL = 10
LC_RANSAC_DIST = 0.5       # RANSAC correspondence distance
LC_RANSAC_FITNESS = 0.20   # Min fitness for RANSAC global registration
LC_ICP_REFINE_DIST = 1.0   # ICP refinement distance after RANSAC
LC_ICP_REFINE_FITNESS = 0.30  # Min fitness for ICP refinement
LC_DEDUP_WINDOW = 50       # Dedup window: 1 closure per (window_i, window_j) pair

# pose graphPG_ODOM_WEIGHT = 1.0
PG_LC_WEIGHT = 5.0
...
# ============================================================================
# sparse Pose Graph Optimizer with LM Damping
# ============================================================================
...
# deduplicate: 1 best per (window_i, window_j) pairdedup = {}
