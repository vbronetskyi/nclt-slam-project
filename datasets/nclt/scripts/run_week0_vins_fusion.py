...
Runs VINS-Fusion (no-ROS build) in mono-inertial mode on NCLT Ladybug3
Camera 0 images across two sessions, then progressively fuses wheel
odometry and GPS constraints via pose graph optimization.

Three configurations evaluated:
  - Config A: Pure VIO (mono + IMU), Sim(3) alignment
  - Config B: VIO + Wheel Odometry fusion (pose graph), SE(3) alignment
  - Config C: VIO + Wheel Odometry + GPS fusion (pose graph), SE(3) alignment
