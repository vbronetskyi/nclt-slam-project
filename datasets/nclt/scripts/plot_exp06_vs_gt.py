...
# sim(3) for panel 3aligned, sim3_scale = umeyama(me, mg)
errs_sim3 = np.linalg.norm(aligned[:, :2] - mg[:, :2], axis=1)
ate_sim3 = np.sqrt(np.mean(errs_sim3**2))

# pathsgt_path = np.sum(np.linalg.norm(np.diff(mg, axis=0), axis=1))
raw_path = np.sum(np.linalg.norm(np.diff(me, axis=0), axis=1))

# shift to start at (0,0) for fair comparisongt_from0 = mg[:, :2] - mg[0, :2]
...
ax.set_title(f'ORB-SLAM3 VIO output (raw, no alignment)\n'
             f'Path: {raw_path:,.0f} m  —  scale error {raw_path/gt_path:.0f}×',
...
ax = axes[2]
# both from common starta0 = aligned[:, :2] - aligned[0, :2]
g0 = mg[:, :2] - mg[0, :2]
# pin starta0_pinned = a0 + (g0[0] - a0[0])
