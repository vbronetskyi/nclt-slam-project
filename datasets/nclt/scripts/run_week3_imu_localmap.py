#!/usr/bin/env python3
"""
Week 3: Improved LiDAR-Inertial Odometry
=========================================
Session: 2012-04-29, every 2nd scan (6,486 scans)

Improvements over Week 2 baseline (ATE 419m):
1. Wheel odometry prediction for ICP initial guess (replaces identity init)
2. Local map matching (sliding window of 20 scans, 0.5m voxel)
3. Ground plane removal (RANSAC) before ICP
4. GPS-aided loop closure detection (RTK GPS proximity)
5. Pose graph optimization (sparse Gauss-Newton + LM damping)

Bug fixes from v1:
- ms25.csv has 10 columns (mag+accel+gyro), NOT quaternions.
  v1 fed magnetometer values to SLERP as quaternions -> garbage rotations.
- Open3D transform() modifies in-place. v1's scan-to-map code
  corrupted pcd objects through double in-place transforms.
- Now uses odometry_mu_100hz.csv (integrated wheel encoder poses)
  for ICP initial guess instead of broken IMU preintegration.
- Scan-to-map ICP uses T_pred as init parameter directly,
  avoiding any in-place transforms.

Target: ATE < 50m (10x improvement)
"""
import os
import sys
import numpy as np
...
# ICP parameters
ICP_VOXEL = 0.3          # Voxel downsampling for current scan
ICP_THRESHOLD = 1.5      # ICP correspondence distance
ICP_MAX_ITER = 80        # ICP iterations

# local map parametersLOCALMAP_SIZE = 20
# Number of past scans in local map
LOCALMAP_VOXEL = 0.5      # Voxel size for local map downsampling

# ground removal parametersGROUND_DIST = 0.25
# RANSAC distance threshold for ground plane
GROUND_RANSAC_N = 3       # RANSAC min points
GROUND_ITERS = 200        # RANSAC iterations

# GPS loop closure parameters
GPS_LC_MIN_GAP = 200      # Minimum frame gap for loop closures
GPS_LC_RADIUS = 15.0      # GPS proximity radius (meters)
GPS_LC_ICP_FITNESS = 0.25 # ICP fitness threshold for verification
GPS_LC_ICP_DIST = 2.0     # ICP distance for loop closure verification
GPS_LC_DEDUP = 50         # Dedup window (frames)

# pose graphPG_ODOM_W = 1.0
PG_LC_W = 10.0
PG_DAMPING = 1e-3
...
# odometry Predictor (replaces broken IMU preintegrator)
...
# pose Graph Optimizer (2D, sparse, LM damping)
...
# metrics
def compute_ate(e, g):
    """ATE — position L2 norm per frame"""
...
def compute_rpe(e, g, d=1):
    """RPE — frame-to-frame transform error"""
    te, re = [], []
...
# load wheel odometry (100Hz integrated poses)odom_df = sensor.load_odometry_mu(SESSION, hz=100)
...
# initialize componentsodom_pred = OdometryPredictor(odom_df)
...
# compute metricsate_odom = compute_ate(odom_traj, gt_traj)
