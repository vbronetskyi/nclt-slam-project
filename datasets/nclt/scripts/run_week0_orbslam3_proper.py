...
Fixes from previous attempts (0.2-0.5):
1. Camera model: PinHole (f=221) → KannalaBrandt8 fisheye
2. IMU: not used → mono-inertial + stereo-inertial with MS25 + KVH hybrid
3. Camera: Cam0 (sky!) → Cam5 (forward), Cam4 (left), Cam1 (right)
4. Mode: Mono only → Mono-Inertial AND Stereo-Inertial
5. Extrinsics: guessed → computed from NCLT calibration + Ladybug3 geometry

Phases:
  A. Calibration and data preparation
  B. Mono-inertial sweep on 3000 frames
  C. Stereo-inertial on 3000 frames
  D. Full spring session with best config
  E. Wheel odometry fusion (post-processing)
  F. Summer session
  G. Results, plots, report

Usage:
    tmux new -s orbslam && python3 scripts/run_week0_orbslam3_proper.py 2>&1 | tee /workspace/orbslam_proper_log.txt
"""
...
# cam0=top(sky), Cam1=right(+73), Cam2=back-right(+144),# cam3=back-left(-143), Cam4=left(-72), Cam5=forward(0)CAM_ANGLES = {1: 73.0, 4: -72.0, 5: 0.0}

# Ladybug3 approximate geometry
LB3_RADIUS = 0.08  # sphere radius in meters
LB3_CENTER_BODY = np.array([0.035, 0.002, -1.23])  # center in body NED frame

# default fisheye calibration (half-res 808x616)# For equidistant model: f = r_max / θ_max = 404 / (60° in rad) ≈ 386
# assuming ~120° horizontal FOV for Ladybug3 side cameras
DEFAULT_FISHEYE = {
...
def phase_c(session, calib, best_imu):
    ...
    for left_id, right_id, pair_label in stereo_pairs:
        ...
        # evaluate (SE(3) alignment — stereo has metric scale)
        if result["success"] and result["traj_path"]:
