...
This project evaluates odometry and SLAM methods on the NCLT dataset (University of Michigan North Campus Long-Term, 4 seasonal sessions, ~3–7 km each). The goal is to find a method that produces accurate, complete trajectories across all seasons.
...
1. **Ladybug3 Cam0 is the TOP camera pointing at the sky.** Experiments 0.2–0.4 unknowingly tracked clouds and tree canopy. This single error invalidates all visual SLAM results from those experiments. Discovered in Experiment 0.5 via systematic camera investigation.

2. **Switching to side cameras (Cam3/4/5) doesn't save visual SLAM.** After testing all 6 cameras with 23 parameter configurations, the best ORB-SLAM3 tracking rate on 3000 frames was only 27.9%, and no trajectory followed the ground truth road shape.

3. **ORB-SLAM3 is extremely non-deterministic on this data.** Same config + same data produces tracking rates from 5% to 70% across runs. Systematic tuning is unreliable without many repetitions per config.

4. **Root causes of visual SLAM failure are hardware-level:**
   - 5 Hz camera (methods expect 20–30 Hz)
   - ~120° FOV fisheye treated as pinhole (no distortion coefficients available)
   - No viable IMU fusion (MS25 at 34 Hz too slow for ORB-SLAM3 VIO)
   - Poor/variable image quality (overexposed, low-texture frames)

5. **LiDAR ICP is the only functional odometry** — 174 m spring / 188 m summer ATE with 100% trajectory coverage. Drift is significant but trajectories are recognizable and complete.
...
- **Conclusion:** Too sparse (every 10th = ~2s gap), poor scan-to-scan overlap
...
- **Conclusion:** 82% RPE improvement vs sparse, but 13% drift/km over 3.3km trajectory accumulates to 419m ATE. No loop closure → unbounded drift.
...
- **Conclusion:** Too few loop closures for meaningful correction. Scan context matching is too conservative on this dataset. Need GPS-aided loop closure or learning-based descriptors
...
- **Root cause:** ms25.csv magnetometer values fed to SLERP as quaternions → random orientations → ICP diverged to wrong local minima → corrupted local map → cascading errors
- **Conclusion:** Completely broken due to data loader bugs. Discarded
...
- **Key improvements vs Week 2:**
  - Odometry prediction provides much better ICP init than identity (prevents local minima)
  - Local map matching gives more geometric context than frame-to-frame
  - Ground removal reduces planar ambiguity
  - GPS loop closures (107 vs 7 from scan context) provide many more global constraints
- **Remaining issues:** 174m ATE still far from <50m target. Drift accumulates in areas without GPS coverage. Loop closures cluster in GPS-dense regions, leaving long unconstrained segments
...
- **Conclusion:** ORB-SLAM3 monocular is not viable on the Ladybug3 camera for NCLT:
  - Ladybug3 is a fisheye camera approximated as pinhole — significant distortion in outer regions
  - Only 23/5000 keyframes tracked on spring session (0.5% success rate)
  - Summer session failed completely (33 map resets, empty trajectory)
  - The 2.9m ATE on 23 poses is misleadingly good — covers only a tiny segment of the trajectory
  - Camera is forward-facing on a vehicle with significant rotational motion → frequent tracking loss
  - Would need proper fisheye/equirectangular camera model and possibly stereo for viable visual SLAM
...
|-----|-------|----------|------------|------|----------|-------|
| Spring 3k (cherry-picked) | 2981/3000 | 99.4% | 791m of 3186m (25%) | 1 | 45.6m | Best 10-min segment only |
...
- **Comparison with LiDAR ICP (NOT directly comparable):**
  ...
  - ORB-SLAM3 best: 45.6m ATE over **791m** cherry-picked best segment (25% of trajectory)
...
- **Key findings:**
  - **Mono-inertial mode is infeasible** on NCLT: low camera FPS (5 Hz), low IMU rate (34 Hz), poor image quality → constant map resets, IMU never fully initializes
  - **Pure monocular works on good image segments**: 99.4% tracking on the best 10-minute segment with aggressive ORB config
  - **Full session tracking is unreliable**: only 24% of spring and 19% of summer images tracked, heavily fragmented across dozens of maps
  - **Image quality is the bottleneck**: first 500 images overexposed, many later images low-texture. Only ~34% of images have ≥500 ORB features
  - Monocular scale is arbitrary (factors 23–137 across runs) and can only be recovered post-hoc via ground truth alignment
- **Conclusion:** ORB-SLAM3 monocular is **not viable for full-session NCLT evaluation**. Tracking is too fragmented (24% spring, 19% summer) and produces disconnected trajectory segments. LiDAR ICP remains the more reliable approach despite higher absolute ATE, because it provides 100% trajectory coverage. The 45.6m ATE on the best 10-min segment demonstrates that visual tracking CAN work on NCLT Ladybug3 images under good conditions, but the camera's inconsistent image quality prevents reliable long-term operation.
...
- **Honest assessment — ALL methods effectively failed:**
  ...
  - **Scale factors reveal catastrophic failure:** DROID-SLAM 3.27× (world appears 3.3× too big), DPVO 0.33× (3× too small), DPV-SLAM 0.12× (8× too small). These are not "monocular scale ambiguity" — these are methods that cannot estimate consistent motion
  ...
  - **ATE vs distance plot shows no structure:** Errors oscillate 50-450m with no pattern — consistent with a non-functional system, not meaningful drift

- **Root causes of failure:**
  1. **5 Hz camera rate** — all three methods designed/trained for 20-30 Hz video; at 5 Hz inter-frame motion is 4-6× larger than expected, breaking optical flow and learned correspondences
  2. **Fisheye distortion unhandled** — Ladybug3 has ~120° FOV fisheye but images were passed raw with pinhole intrinsics (f=221); outer-region distortion corrupts feature matching
  3. **No undistortion possible** — NCLT `cam_params.zip` is unavailable (website 404, S3 403), so undistortion maps cannot be applied
  4. **Poor image quality** — first ~500 images overexposed (mean pixel ≈ 255), many frames low-texture
  5. **Training domain mismatch** — DROID-SLAM/DPVO trained on indoor (TartanAir) and standard datasets; NCLT outdoor fisheye is far out of distribution

- **Comparison with ORB-SLAM3 (Experiment 0.3):**
  - ORB-SLAM3's best tracked segments (45.6m ATE on 791m) actually follow the GT road shape locally — deep methods do not
  - Deep methods track more frames (DPVO 100%) but produce globally incoherent trajectories
  - ORB-SLAM3 fails by losing tracking (map resets); deep methods fail by tracking confidently but wrongly

- **Conclusion:** Deep visual SLAM methods (DROID-SLAM, DPVO, DPV-SLAM) are **not viable on NCLT Ladybug3 data**. The 5 Hz fisheye camera at 808×616 is fundamentally incompatible with methods designed for 20+ Hz pinhole cameras. The ATE numbers (110-194m) should NOT be interpreted as meaningful accuracy — they are artifacts of Sim(3) alignment on wrong-shaped trajectories. LiDAR ICP (174-188m ATE) remains the only method that produces recognizable trajectories on this dataset, despite its own significant drift
...
- **Critical discovery:** Ladybug3 Cam0 is the TOP camera (sky-facing). ALL previous visual SLAM experiments (0.2, 0.3, 0.4) used sky images — this was the root cause of catastrophic visual SLAM failures.
- **Method:** 4-phase systematic investigation:
...
- **Key findings:**
  - Maximum tracking rate on 3000 frames: **27.9%** (Cam4) — far below viable threshold
  - **Extreme non-determinism:** Same config produces 2–4× different tracking rates across runs (Cam5+CLAHE: 29%–70%)
  - **More features ≠ better tracking:** nFeatures=5000 gave worst result (0.9%); Cam1 with 4895 ORB features tracked only 7.6%
  - **CLAHE is unreliable:** Large improvements on 1000 frames vanished on 3000 frames
  - **No trajectory follows GT road shape:** All estimated paths are chaotic blobs regardless of parameters
  - **Full session timed out:** Phase 3 on 21510 frames exceeded 45-minute limit
- **Conclusion:** ORB-SLAM3 monocular is NOT viable on NCLT Ladybug3. Root causes (5 Hz fisheye camera, unmodeled distortion, no IMU fusion, extreme non-determinism) cannot be overcome through parameter tuning. LiDAR ICP (174m ATE, 100% coverage) remains the only functional odometry method on this dataset
...
- **Key findings:**
  - **Scale near 1.0 (0.97):** Stereo provides correct metric scale — only 3.4% error
  - **Trajectory follows GT road shape:** Unlike NCLT visual SLAM experiments, this trajectory clearly matches the ground truth route
  - **3.91m ATE RMSE is reasonable** for stereo-only SLAM over 834m without IMU/GPS fusion
  - **72.7% tracking:** Lost final 27% of sequence (111s) due to accumulated map fragmentation
  - **RPE at 1s (6.7m) is high** due to relocalization discontinuities — trajectory has jump artifacts from map switches
  - **Stereo-Inertial mode is not viable without raw IMU data** — pseudo-IMU from INS causes systematic failure in ORB-SLAM3's tightly-coupled VI optimizer
  - **Comparison with NCLT results:** RobotCar stereo works dramatically better than NCLT monocular fisheye because: (a) stereo provides metric scale, (b) 16 Hz vs 5 Hz camera, (c) pre-undistorted pinhole images, (d) structured urban scenes with good texture
...
- **Analysis:**
  - **Best result so far** — sub-meter ATE on a 3.7 km route
  - **Scale 0.997** confirms real IMU integration works correctly (vs pseudo-IMU failure in Exp 0.8)
  - **99.99% tracking** — near-perfect, only 2 lost frames out of 15177
  - **Real 2000 Hz IMU is the key differentiator** vs RobotCar's pseudo-IMU: proper pre-integration, correct motion estimation, stable VIBA optimization
  - **Loop closures detected** but some rejected ("BAD LOOP") due to large rotation discrepancy — tuning thresholds could improve further
  - **Comparison with Exp 0.8 (RobotCar Stereo-only):** 4× better ATE (0.93 vs 3.91 m), near-perfect tracking (99.99% vs 72.7%), correct scale (0.997 vs 0.966)
