...
Scan context descriptors, FPFH+RANSAC verification, sparse 2D pose graph with LM damping
"""
import numpy as np
...
class ScanContext:
    ...
    2D descriptor (rings x sectors) with max height per bin.
    """
    ...
    def ring_key(self, desc):
        """Compute ring key (mean per ring) for fast pre-filtering"""
...
class PoseGraphOptimizer2D:
    ...
    Optimizes [x, y, theta] with odom + LC edges, first pose anchored.
    """
