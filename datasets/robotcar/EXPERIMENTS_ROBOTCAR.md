...
Oxford RobotCar is a research project from the University of Oxford in which a **Nissan LEAF** equipped with a full sensor suite drove the same route (~10 km through central Oxford) over 100 times throughout the year (November 2014 -- November 2015). This provides a unique opportunity to compare localization performance under varying conditions: rain, snow, night, dawn, sunset -- all along the same route.
...
**Important:** The RobotCar Seasons benchmark uses only the **3 monocular cameras** (Grasshopper2 -- left, rear, right). The images are already **undistorted** (corrected from fisheye distortion) and cropped to a pinhole model with parameters fx=fy=400. The XB3 stereo camera is a separate camera that was used in our ORB-SLAM3 experiment.
...
|-----------|---------------|-----------------------------------|
| **overcast-reference** | 28.11.2014 | Reference map. 20,862 images across 49 locations |
...
We use the **hloc** pipeline (Hierarchical Localization, Sarlin et al. 2019) -- the standard approach for visual localization:
...
- Matched reference images have a very similar appearance
...
A Stereo-Inertial mode of ORB-SLAM3 was also attempted, but:
- **Problem:** The raw IMU data is not published in the RobotCar dataset
- **Approach:** We synthesized pseudo-IMU by differentiating INS velocities/orientations
- **Result:** Complete failure -- the pseudo-IMU is too "smooth" for initialization of tightly-coupled VI fusion; the system diverges after VIBA 2 (segfault)
- **Conclusion:** Pseudo-IMU is fundamentally incompatible with tightly-coupled VIO
...
**Key conclusion:** These are **different tasks**. hloc answers "where am I relative to a known map?", while ORB-SLAM3 answers "how am I moving right now?". In a real system, they complement each other.
...
1. **ALIKED+LightGlue+OpenIBL** -- best combination for visual localization (64.5% @ 0.5m/5)
2. **OpenIBL** instead of NetVLAD gives +10% at night (40.6% vs 30.9%) -- a critical difference
3. **Learned detectors** (SuperPoint, ALIKED, DISK) significantly outperform SIFT (+6-8% overall)
4. **LightGlue is comparable to SuperGlue** in quality but faster -- can replace it without loss
5. **Stereo SLAM** on RobotCar works (3.91m ATE) -- the first successful visual SLAM in the project

### 5.2 What Does Not Work

1. **Nighttime localization** -- all methods drop to 30-40% (from 70%+ during the day). A ~30 pp gap -- the biggest challenge
2. **SIFT at night** -- practically useless (5.6% @ 0.5m/5)
3. **Pseudo-IMU** for ORB-SLAM3 -- fundamentally incompatible with tightly-coupled VIO
4. **ORB-SLAM3** does not support cross-season localization -- it only works with the current session
