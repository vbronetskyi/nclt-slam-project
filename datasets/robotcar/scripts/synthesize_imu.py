#!/usr/bin/env python3
"""Synthesize pseudo-IMU from RobotCar ins.csv (no raw accel/gyro available)

Derives angular velocity from Euler angle differentiation,
and specific force from velocity differentiation + gravity rotation.
Output: EuRoC CSV format (timestamp_ns, gx, gy, gz, ax, ay, az)
"""
...
def euler_rates_to_body_angular_velocity(roll, pitch, d_roll, d_pitch, d_yaw):
    """convert Euler angle rates to body-frame angular velocity (ZYX)"""
...
def synthesize_imu(ins_data, gravity=9.81007, smooth_window=5):
    ...
    # average dt for uniform operations
    dt_mean = np.mean(dt)
    ...
    # sanity check: at rest, accelerometer should read ~[0, 0, -g] in NED body
    # which is [0, 0, +g] in z-up convention
