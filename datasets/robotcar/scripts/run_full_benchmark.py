#!/usr/bin/env python3
"""
RobotCar Seasons Full Benchmark — Feature Extractor & Matcher Ablation

Runs multiple hloc pipeline configurations on the RobotCar Seasons benchmark
and evaluates all results. Designed for unattended overnight execution.

Methods tested:
  1. SuperPoint + SuperGlue + NetVLAD      (baseline, already computed)
  2. SuperPoint + LightGlue + NetVLAD      (faster matcher)
  3. DISK + LightGlue + NetVLAD            (alternative learned features)
  4. ALIKED + LightGlue + NetVLAD          (recent efficient features)
  5. SuperPoint + SuperGlue + OpenIBL       (alternative retrieval)
  6. SIFT + NN-ratio + NetVLAD             (classical baseline)

Usage:
  tmux new -s hloc_full
  python3 /workspace/datasets/robotcar/scripts/run_full_benchmark.py 2>&1 | tee /workspace/robotcar_hloc_full_log.txt
"""
...
# fallback: use SP+SG reference SfM if triangulation crashes for other featuresFALLBACK_SFM = 'sfm_superpoint+superglue'

# ============================================================
# logging — only stdout, no FileHandler (tee handles file output)
# ============================================================
logging.basicConfig(
    level=logging.INFO,
...
# signal name lookup for crash diagnosticsSIGNAL_NAMES = {v: v.name for v in signal.Signals}
