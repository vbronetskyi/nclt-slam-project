...
**4Seasons** is a dataset from the Technical University of Munich (TUM), created for evaluating visual localization and SLAM under changing seasonal conditions. A fully sensor-equipped vehicle drove the same route ("office loop" — a loop around an office complex) during different seasons: spring, summer, and winter. This allows comparing algorithm performance under varying lighting, vegetation, and weather conditions.

Unlike RobotCar, 4Seasons has a **real high-frequency IMU** (2000 Hz), which enables testing Stereo-Inertial SLAM — something that was not possible on RobotCar due to the absence of raw IMU data.
...
|------|-------------|--------|------|------|
| **office_loop_1** | recording_2020-03-24_17-36-22 | Spring | 24.03.2020 | Reference session. Mild weather, leaves beginning to grow |
...
**Important note:** GNSS and VIO use **different coordinate systems**. VIO poses are local (with drift), while GNSS poses are global (with noise from satellite measurements). Comparison requires Sim3 alignment (scale + rotation + translation).

### 1.6 Ground Truth: VIO vs GNSS

The dataset has **two sources of ground truth**, and understanding this distinction is critical:
...
**Conclusion:** For **self-test** (localization within the same session), one should compare against **VIO**, not GNSS. For **cross-season** (localizing a different session), one should compare against **GNSS** via Sim3 alignment. This distinction was a key mistake that we corrected (see Section 4.3).
...
On RobotCar we achieved **3.91m ATE** with pure Stereo SLAM, but were unable to run Stereo-Inertial — the raw IMU is not published, and a pseudo-IMU (synthesized from INS) does not work with tightly-coupled VIO.

4Seasons has a **real ADIS16465 IMU at 2000 Hz** — ideal conditions for testing Stereo-Inertial SLAM. This allows answering the question: how much does an IMU contribute to SLAM accuracy?
...
**Important ORB-SLAM3 bug:** In the Settings.cc code (Settings path), `IMU.fastInit` **is not read** — only `IMU.Frequency`, noise parameters, and T_b_c are parsed. We added a fix for this.
...
- Stable operation without catastrophic drift
...
**Why 0.93m is a very good result:**

1. **4x better than RobotCar** (0.93m vs 3.91m) with a similar route length
2. **Scale 0.997** — IMU + stereo yield practically perfect metric scale
3. **Low RPE (0.40m)** — local accuracy is excellent, indicating stable tracking
4. **ATE Max 3.06m** (vs 6.75m on RobotCar) — fewer extreme deviations

**Why is the tracking rate only 23.3%?**
- This **does not mean** tracking occurred only 23% of the time. ORB-SLAM3 saves only **keyframes**, not all frames
- 3,536 keyframes out of 15,177 frames = 23.3% — normal for Stereo-Inertial
- Between keyframes the system tracks, but does not save every frame to the trajectory
- With full-frame trajectory (f_*.txt) the tracking rate is significantly higher
...
**Conclusion:** The IMU is a critical component. It stabilizes tracking, improves scale, and reduces drift. The absence of an IMU on RobotCar is the main reason for its lower accuracy.
...
During pipeline development, **3 critical bugs** were identified and fixed:

**Bug #1 — pairs_from_retrieval without query_list:**
- **Symptom:** `KeyError` in triangulation (COLMAP does not know query images)
- **Cause:** `pairs_from_retrieval.main()` without `query_list` matches ALL 8,510 images (including queries) against ref
- **Fix:** Added `query_list=ref_list_path` to produce correct 34,140 ref-ref pairs
...
- **Cause:** Compared localized poses (in VIO frame) against GNSS GT via Sim3 alignment. However, VIO-to-GNSS alignment has 5.98m RMSE — this noise was added to the result
- **Fix:** Created `evaluate_self_test()` function that compares directly against VIO poses (same coordinate system). Result: 3.76m median (instead of 6.83m)
...
**Conclusion:** ORB-SLAM3 Stereo-Inertial is **4-5 times more accurate** than hloc on the same session, because it uses:
1. Continuous data stream (not a single image)
2. IMU for stabilization
3. Stereo for depth
4. Local BA for optimization

However, hloc has a key advantage — **cross-season** localization (testing in progress).
...
1. **0.93m ATE RMSE** — the best result across the entire project (vs 3.91m RobotCar, vs 174m NCLT LiDAR)
2. **IMU is a critical component:** 4x ATE improvement compared to pure stereo (0.93m vs 3.91m)
3. **Real IMU vs pseudo-IMU:** Synthesized IMU (RobotCar) does not work. A real 2000 Hz ADIS16465 works excellently
4. **Scale 0.997** — nearly perfect thanks to stereo + IMU fusion
5. **Stable worst-case behavior:** ATE does not exceed 3m even in the worst locations

### 6.2 hloc Localization

1. **Self-test limited by 3D model density:** 1,707 ref images producing 61,688 3D points — insufficient for dense coverage
2. **43% of queries with error >5m** — in locations without sufficient 3D points
3. **17% of queries with error <0.5m** — where sufficient inliers exist, accuracy is very high
4. **NetVLAD is equivalent to OpenIBL** on same-season data (differences appear only under changing conditions)
5. **PnP inliers are the best quality predictor:** >=200 inliers yields 0.59m median
