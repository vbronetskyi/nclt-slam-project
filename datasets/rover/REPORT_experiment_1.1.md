...
**Symptom:** Same as Stereo — the visual front-end cannot initialize, so IMU pre-integration has no visual anchor. ORB-SLAM3's Stereo-Inertial mode requires successful stereo initialization before fusing IMU data.
...
3. **Less distinctive landmarks** in the park environment (trees, grass are self-similar)
