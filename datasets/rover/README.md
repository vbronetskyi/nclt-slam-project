...
- Python 3.10+
- ORB-SLAM3 built at `../third_party/ORB_SLAM3/` (with vocabulary at `Vocabulary/ORBvoc.txt`)
- Xvfb (for headless ORB-SLAM3 viewer): `apt install xvfb`
- Python packages: `pip install numpy matplotlib opencv-python scipy`
- Dataset: download all 15 recordings from [iis-esslingen/ROVER](https://huggingface.co/datasets/iis-esslingen/ROVER) into `../data/rover/`

### How to Run

```bash
# 1. convert T265 stereo data to EuRoC format
python3 scripts/convert_rover_to_euroc.py --recording garden_large_day

# 2. undistort T265 fisheye to pinhole (needed for stereo/SI modes)
python3 scripts/rectify_t265_stereo.py --recording garden_large_day

# 3. convert D435i data to TUM RGB-D format
...
# to run ALL 15 recordings overnight (starts Xvfb, runs in parallel):
bash scripts/run_overnight.sh
```
...
**Garden Large summary:** mean ATE **0.414 m**, median **0.409 m**. Results are highly consistent -- all 6 successful runs fall within a narrow range of 0.37--0.48 m. Seasonal variation (summer/autumn/winter/spring) has **negligible impact** on RGB-D SLAM accuracy.

**Park (5 of 7 successful):**
