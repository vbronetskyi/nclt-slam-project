...
Provides composable transforms for training place recognition models
...
class RandomFlip:
    """Randomly flip point cloud along X or Y axis.
...
class RandomSubsample:
    """Randomly subsample points to a fixed count.
...
class Normalize:
    """Center point cloud at origin and optionally scale.

    Args:
        center: Whether to center at origin.
        scale: Whether to normalize to unit sphere.
    """
...
class RemoveGround:
    """Remove ground plane points by height threshold.
...
def build_transforms(config: dict, is_train: bool = True) -> Compose:
    ...
    Args:
        config: Augmentation config section from train_config.yaml.
        is_train: Whether to include randomized augmentations.
