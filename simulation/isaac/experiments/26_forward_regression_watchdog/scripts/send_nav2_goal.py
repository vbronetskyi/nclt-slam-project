...
class Nav2TwoPhase(Node):
    ...
    def get_lateral_deviation(self, rx, ry):
        """Perpendicular distance from robot to the path (nearest segment).

        Only returns a meaningful "lateral" value when the robot's projection
        falls ONTO a segment (0 < t < 1). If the robot is behind the first
        segment or past the last, returns 0 - this means the robot is lagging
        along the path direction, not drifting sideways.
        """
        wps = self._current_wps()
        if len(wps) < 2:
            return 0.0
        # Check a window of segments around current_idx
        ...
        for i in range(start, end):
            ...
            if length2 < 0.01:
                continue
            t = ((rx - wx1) * dx + (ry - wy1) * dy) / length2
            if t < 0 or t > 1:
                continue  # robot not ON this segment
            closest_x = wx1 + t * dx
            closest_y = wy1 + t * dy
            d = math.hypot(rx - closest_x, ry - closest_y)
            if d < best:
                best = d
                found = True
    ...
    def _handle_regression(self, rx, ry):
        """Forward-regression recovery: clear costmaps, skip waypoints ahead.
        ...
        # 2. Skip waypoints: find first wp at least FORWARD_SKIP_DISTANCE ahead
        wps = self._current_wps()
        ...
        self.get_logger().warn(
            f'  advanced wp {old_idx+1} -> {self.current_idx+1} '
    ...
    def monitor_tick(self):
        ...
        # Lateral corridor enforcement
        lat_dev = self.get_lateral_deviation(rx, ry)
        if self._corridor_state == 'NAVIGATING':
            if lat_dev > MAX_LATERAL_DEV:
                ret_x, ret_y = self.get_return_point(rx, ry)
                self.get_logger().warn(
                    f'LATERAL DEV {lat_dev:.1f}m > {MAX_LATERAL_DEV}m, '
                    f'returning to path ({ret_x:.1f},{ret_y:.1f})')
