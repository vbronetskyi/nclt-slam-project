...
WEB_MAP_PATH = '/workspace/simulation/isaac/results/final/01_web_map.png'
WEB_MAP_EXTENT = [-120, 120, -80, 80]  # X_MIN, X_MAX, Y_MIN, Y_MAX in world coords
...
def plot_trajectories(trajs, labels, title, output_path, show_obstacles=True):
    ...
    Layout:
      - Top: Y vs X with road curve overlay (shows actual road shape)
      - Middle: web map with trees + traveled route
    ...
    ax_road.fill_between(road_x_dense, road_y_dense - 2, road_y_dense + 2,
                         color='#888', alpha=0.15, label='road ±2m')
    ...
    if show_obstacles:
        for i, group in enumerate(CONE_GROUPS):
            for cx, cy in group:
                ax_road.plot(cx, cy, 'o', color='#FF9800', markersize=6, zorder=5)
        ax_road.plot([], [], 'o', color='#FF9800', markersize=6, label='cones')
        ax_road.add_patch(mpatches.FancyBboxPatch(
            (TENT_POS[0]-1.5, TENT_POS[1]-1), 3, 2,
            boxstyle="round,pad=0.2", facecolor='#795548', alpha=0.7, zorder=5))
    ...
    for i, (traj, label) in enumerate(zip(trajs, labels)):
        ...
        if np.any(outbound):
            ax_road.plot(x[outbound], y[outbound], color=color,
                         linewidth=1.8, alpha=0.9, zorder=7,
                         label=f'{label} (outbound)')
        if np.any(ret):
            ax_road.plot(x[ret], y[ret], color=color,
        ...
        ax_road.annotate(f'{lat_err[max_idx]:+.1f}m',
                         (x[max_idx], y[max_idx]),
    ...
    ax_road.plot(*SPAWN, 's', color='#4CAF50', markersize=10, zorder=10, label='start')
    ax_road.plot(*DEST, '*', color='#F44336', markersize=12, zorder=10, label='dest')
    ax_road.set_xlabel('X (m)', fontsize=10)
    ax_road.set_ylabel('Y (m)', fontsize=10)
    ax_road.set_title('Robot trajectory on road (curve shape preserved)',
                      fontsize=10, style='italic')
    ax_road.set_xlim(-105, 85)
    ax_road.set_ylim(-15, 12)
    ax_road.legend(loc='upper left', fontsize=8, framealpha=0.9)
    ...
    if os.path.exists(WEB_MAP_PATH):
        ...
        ax_map.imshow(bg, extent=WEB_MAP_EXTENT, aspect='equal',
                      alpha=0.85, zorder=1)
    else:
    ...
    if show_obstacles:
        for group in CONE_GROUPS:
            for cx, cy in group:
                ax_map.add_patch(plt.Circle((cx, cy), 0.6, color='#FFEB3B',
                                            alpha=0.95, zorder=5))
        ax_map.plot([], [], 'o', color='#FFEB3B', markersize=8, label='cones')
        ax_map.add_patch(mpatches.FancyBboxPatch(
            (TENT_POS[0]-1.5, TENT_POS[1]-1), 3, 2,
            boxstyle="round,pad=0.3", facecolor='#FF5722',
            edgecolor='white', linewidth=1, alpha=0.9, zorder=5))
    ...
    for i, (traj, label) in enumerate(zip(trajs, labels)):
        ...
        if np.any(outbound):
            ax_map.plot(x[outbound], y[outbound], color=color,
                        linewidth=2.5, alpha=0.95, zorder=7,
                        label=f'{label} (outbound)')
        if np.any(ret):
            ax_map.plot(x[ret], y[ret], color=color,
                        linewidth=2, alpha=0.6, linestyle='--', zorder=7,
                        label=f'{label} (return)')
        ...
        if np.any(outbound):
            ...
            ax_map.plot(ox[max_idx], oy[max_idx], 'D', color=color, markersize=10,
                        zorder=9, markeredgecolor='white', markeredgewidth=1.8)
            ax_map.annotate(f'max ({ox[max_idx]:.0f},{oy[max_idx]:.0f})',
                            (ox[max_idx], oy[max_idx]),
    ...
    ax_map.plot(*SPAWN, 's', color='#4CAF50', markersize=14, zorder=10,
                markeredgecolor='white', markeredgewidth=1.5, label='start')
    ax_map.plot(*DEST, '*', color='#F44336', markersize=18, zorder=10,
                markeredgecolor='white', markeredgewidth=1.5, label='destination')

    ax_map.set_xlabel('X (m)', fontsize=10)
    ax_map.set_ylabel('Y (m)', fontsize=10)
    ax_map.set_title('Scene map (trees + obstacles + traveled route)',
                     fontsize=10, style='italic')
    ax_map.set_xlim(-110, 85)
