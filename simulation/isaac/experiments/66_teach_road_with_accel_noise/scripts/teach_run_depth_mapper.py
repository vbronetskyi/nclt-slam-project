...
TF tree. For each depth frame:
  1. Look up transform map -> camera_link (from the VIO-driven TF chain)
  2. Transform all points into map frame
  3. Height filter: keep points with 0.2 m < z < 2.0 m (exclude ground/canopy)
  4. For each point: 2D raytrace (Bresenham) from robot's (x,y) to the
     projected (x,y) of the point - cells along the way get "free" votes,
     the endpoint cell gets an "occupied" vote.

At Ctrl+C / SIGTERM: threshold the accumulated log-odds grid and save
as Nav2 map_server format (pgm + yaml). Only the depth camera observations
are used - no ground-truth scene data is read.

This is the teach-run mapper, so robot drives once through the route and
this builds a snapshot of what the camera actually observed.
"""
import argparse
import os
import signal
...
def _tf_to_matrix(tf_msg):
    """Convert geometry_msgs/TransformStamped to 4x4 numpy."""
...
class TeachDepthMapper(Node):
    ...
    def _bresenham_mark(self, r0, c0, r1, c1):
        """Mark cells along line free, endpoint occupied."""
        dr = abs(r1 - r0)
        dc = abs(c1 - c0)
