...
EOF
chmod +x /tmp/exp65_tf.sh
nohup bash /tmp/exp65_tf.sh > $OUT/tf_slam.log 2>&1 &
...
cat > /tmp/exp65_drift.sh <<EOF
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export ROS_DOMAIN_ID=$DOMAIN
exec python3 $E65/scripts/vio_drift_monitor.py \
    --out $OUT/vio_pose_dense.csv --max-drift-m $MAX_DRIFT_M \
    --check-interval-s 10 --settling-s 30
EOF
chmod +x /tmp/exp65_drift.sh
nohup bash /tmp/exp65_drift.sh > $OUT/drift_monitor.log 2>&1 &
DRIFT=$!; disown $DRIFT; echo "DriftMonitor: $DRIFT"

cat > /tmp/exp65_depth.sh <<EOF
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=$DOMAIN
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec python3 $E65/scripts/teach_run_depth_mapper.py \
    --out-prefix $OUT/teach_map \
    --origin-x $ORIG_X --origin-y $ORIG_Y --width-m $W --height-m $H --res 0.1
EOF
chmod +x /tmp/exp65_depth.sh
nohup bash /tmp/exp65_depth.sh > $OUT/teach_depth_mapper.log 2>&1 &
DEPTH=$!; disown $DEPTH; echo "DepthMapper: $DEPTH"

cat > /tmp/exp65_lm.sh <<EOF
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=$DOMAIN
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec python3 $E65/scripts/visual_landmark_recorder.py \
    --out $OUT/landmarks.pkl --min-disp 2.0
EOF
chmod +x /tmp/exp65_lm.sh
nohup bash /tmp/exp65_lm.sh > $OUT/landmark_recorder.log 2>&1 &
