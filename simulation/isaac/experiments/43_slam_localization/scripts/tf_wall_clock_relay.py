...
class TFRelay(Node):
    ...
    def _tick_slam_encoder(self, now, x, y, z, qx, qy, qz, qw):
        ...
        if slam_ok:
            ...
            # Sanity check: reject large jumps between SLAM and encoder (>5m)
            slam_enc_diff = math.hypot(slam_nx - self.enc_x, slam_ny - self.enc_y)
            if slam_enc_diff > 5.0:
                # SLAM disagrees with encoder significantly -> likely SLAM drift
                slam_ok = False

        if slam_ok:
            # Fuse: 70% SLAM + 30% encoder for position
            SLAM_ALPHA = 0.7
            nav_x = SLAM_ALPHA * slam_nx + (1 - SLAM_ALPHA) * self.enc_x
            nav_y = SLAM_ALPHA * slam_ny + (1 - SLAM_ALPHA) * self.enc_y
            # Use compass heading - SLAM yaw from SE(3) projection is unreliable
