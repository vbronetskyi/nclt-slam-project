...
# aggressive skip parameters
SKIP_FORWARD_MARGIN = 2.0   # m - if robot passed waypoint X by this much, skip
SKIP_TIMEOUT = 15.0          # s - if stuck on one waypoint this long, skip
MAX_NAV2_RETRIES = 2         # max failures before skip (was 5)
CHECK_INTERVAL = 2.0
...
class Nav2TwoPhase(Node):
        ...
        if self.phase == "outbound" and self.current_idx < len(self.outbound_wps):
            ...
            if rx > wx + SKIP_FORWARD_MARGIN:
                ...
                while (self.current_idx < len(self.outbound_wps) and
                       rx > self.outbound_wps[self.current_idx][0] + SKIP_FORWARD_MARGIN):
                    self.current_idx += 1
    ...
    def _cancel_and_send_next(self):
        """Cancel current goal and send next."""
