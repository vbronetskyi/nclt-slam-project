...
log_path = (f"/workspace/simulation/isaac/experiments/30_teach_and_repeat/logs/"
            f"odom_imu_{args.route}_{int(time.time())}.csv")
log_f = open(log_path, "w")
log_f.write("time,gt_x,gt_y,gt_yaw,odom_x,odom_y,odom_yaw,anchor_id,"
            "odom_s,vis_conf,cmd_lin,cmd_ang\n")

# Encoder-based odometry state
odom_yaw = anchors[0]["yaw"]
odom_x = anchors[0]["x"]
odom_y = anchors[0]["y"]
gyro_yaw = anchors[0]["yaw"]
...
# Depth navigator (VFH-style)
from depth_navigator import DepthNavigator
depth_nav = DepthNavigator()
DEPTH_EVERY = 3
last_depth_lin = 0.0
last_depth_ang = 0.0
last_depth_info = ""
...
try:
    while sim_time < args.duration and not follower.finished:
        ...
        # 5. DEPTH NAVIGATOR - primary cmd_vel (skip 2s warmup)
        if frame_count > 120 and frame_count % DEPTH_EVERY == 0:
            depth_img = get_depth()
            if depth_img is not None:
                last_depth_lin, last_depth_ang = depth_nav.compute_cmd_vel(
                    depth_img, heading_err)
                dists = depth_nav.analyze_depth(depth_img)
                last_depth_info = depth_nav.debug_info(dists, heading_err)

        cmd_lin = last_depth_lin
        cmd_ang = last_depth_ang
        ...
        if stuck_recovery_phase > 0:
            ...
            if stuck_recovery_phase == 1:
                ...
                cmd_lin, cmd_ang = last_depth_lin, last_depth_ang
        ...
        if int(sim_time) % 10 == 0 and abs(sim_time - int(sim_time)) < dt:
            ...
            print(
                ...
                f"cmd=({cmd_lin:.2f},{cmd_ang:+.2f}) {last_depth_info}")
