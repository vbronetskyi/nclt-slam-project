...
# Scene adjustments for depth-based navigation:
# 1. Trees: remove every 3rd (33%) - moderate thinning
# 2. Fallen trees: deactivate (full tree models with leaves block depth)
# 3. Cover: keep shrubs, remove ferns (tall, block camera)
print("adjusting scene for depth navigation...")
_thin_count = 0

# 1. Trees: remove every 3rd
for prefix in ["/World/Trees/tree_", "/World/TreeCollision/col_"]:
    idx = 0
    while True:
...
# 2. Fallen trees: deactivate all (they are full tree models rotated 90°
#    with leaves/canopy that create walls for depth camera)
idx = 0
while True:
    path = f"/World/Fallen/fallen_{idx:03d}"
    prim = stage.GetPrimAtPath(path)
    if not prim.IsValid():
        break
    prim.SetActive(False)
...
while True:
    path = f"/World/Cover/cover_{idx:04d}"
    prim = stage.GetPrimAtPath(path)
    if not prim.IsValid():
        break
    prim.SetActive(False)
...
print(f"  deactivated {_thin_count} prims (trees 33% + fallen + ferns/leaves)")

# Also thin gazebo_models.json for collision list (match 33% tree removal)
...
while sim_time < args.duration / 2 and wp_idx < len(teach_waypoints):
    ...
    if last_ax is None or math.hypot(rx - last_ax, ry - last_ay) >= 2.0:
        ...
        if rgb is not None:
            ...
            _rm = "/workspace/simulation/isaac/route_memory/south"
...
log_path = (f"/workspace/simulation/isaac/experiments/32_robust_anchoring/logs/"
            f"robust_thin50_{args.route}_{int(time.time())}.csv")
...
try:
    while sim_time < args.duration and not follower.finished:
        ...
        if frame_count % VISUAL_EVERY == 0:
            ...
            if vis_conf < 0.4:
                gap_nav.MAX_LINEAR = 0.35
                gap_nav.SLOW_SAFE_LINEAR = 0.15
                follower.lookahead_dist = 4.0
            else:
