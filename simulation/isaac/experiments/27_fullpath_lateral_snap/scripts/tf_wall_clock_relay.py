...
ODOM_POS_ALPHA = 0.3
SLAM_YAW_ALPHA = 0.15
# SLAM atlas reset detection: jumps >5m in one frame = reset, record offset
SLAM_RESET_JUMP = 5.0
STAMP_HOLD = 3.0  # seconds to hold fused pose after atlas reset
GYRO_DEADZONE = 0.08
GYRO_LPF_ALPHA = 0.3
JUMP_THRESHOLD = 0.5
...
class TFRelay(Node):
    def __init__(self, use_gt=False, slam_frame=False):
        ...
        self.slam_reset_count = 0
        self.slam_reset_until = 0.0  # wall time; hold position until past this
    ...
    def _tick_slam_frame(self, now, x, y, z, qx, qy, qz, qw):
        ...
        # Atlas reset detection: huge jump = ORB-SLAM3 lost tracking and reset
        # origin. Compute new offset so fused pose stays continuous.
        if slam_jump > SLAM_RESET_JUMP:
            # After reset, raw_nav_* is the new SLAM origin. We want the fused
            # pose to still equal the previous fused pose. So:
            ...
            self.get_logger().warn(
                f'SLAM ATLAS RESET #{self.slam_reset_count}: '
                f'raw jumped by {slam_jump:.1f}m, '
                f'new_offset=({new_offset_x:.1f},{new_offset_y:.1f}), '
                f'holding pose {STAMP_HOLD}s')
            self.slam_offset_x = new_offset_x
            self.slam_offset_y = new_offset_y
            self.slam_offset_yaw = new_offset_yaw
            # Cooldown: don't update fused from SLAM for 3 seconds
            self.slam_reset_until = pytime.time() + STAMP_HOLD
            # Reset tracking refs to new offset-adjusted pose
            self.prev_slam_nx = raw_nav_x + new_offset_x
            self.prev_slam_ny = raw_nav_y + new_offset_y
            self.prev_slam_nyaw = normalize_angle(raw_nav_yaw + new_offset_yaw)
            # Don't fuse this tick, fused stays at old value
            return

        # During cooldown after reset: don't trust SLAM yet, rely on odom/IMU
        in_cooldown = pytime.time() < self.slam_reset_until
