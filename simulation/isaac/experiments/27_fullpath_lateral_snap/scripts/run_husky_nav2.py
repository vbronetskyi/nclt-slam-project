...
parser.add_argument("--use-vio", action="store_true", help="use RGBD-Inertial SLAM (instead of RGBD-only)")
...
if args.use_slam:
    if args.use_vio:
        _slam_binary = "/workspace/third_party/ORB_SLAM3/Examples/RGB-D-Inertial/rgbd_inertial_live"
        # VIO localization mode (loads atlas built offline in exp 19)
        _slam_config = "/root/bags/husky_real/rgbd_inertial_localization_v2.yaml"
    else:
...
if args.use_vio:
    INIT_PHASES = [
        (2.0, 0.0, 0.0),     # stationary - bias estimation
        (1.0, 0.5, 0.5),     # forward + left
        (1.0, 0.8, -0.5),    # faster + right
        (1.0, 0.3, 0.4),     # slow + left
        (1.0, 0.0, 0.0),     # stop (braking)
        (1.0, 0.6, -0.4),    # forward + right
        (1.0, 0.4, 0.4),     # forward + left
        (1.0, 0.7, -0.3),    # faster + right
        (1.0, 0.2, 0.3),     # slow + left
        (1.0, 0.0, 0.0),     # final stop
    ]
    print("\n=== VIO INIT PHASE: 2s stationary + 10s zigzag ===")

try:
    # Run init phases first (sim_time advances)
    for init_dur, init_lin, init_ang in INIT_PHASES:
        phase_end = sim_time + init_dur
        while sim_time < phase_end:
            app.update()
            sim_time += physics_dt
            step += 1
            # apply init cmd
            v_l = (init_lin - init_ang * track / 2) / wheel_r
            ...
            if _imu_reading.is_valid:
                ...
                gt_yaw_init = math.atan2(rot[0][1], rot[0][0])
                qz_h = math.sin(gt_yaw_init / 2)
                qw_h = math.cos(gt_yaw_init / 2)
                _imu_buffer.append(
                    f"{sim_time:.6f} {gx:.6f} {gy:.6f} {gz:.6f} "
                    f"{ax:.6f} {ay:.6f} {az:.6f} 0.0 0.0 {qz_h:.6f} {qw_h:.6f}")
                ...
                if step % 3 == 0:
                    with open(POSE_FILE, 'w') as pf:
                        pf.write(f"{pos[0]:.6f} {pos[1]:.6f} {pos[2]:.6f} 0.0 0.0 {qz_h:.6f} {qw_h:.6f}\n")
            # Record SLAM frames during init too
            if step % 6 == 0 and rec_dir is not None:
                try:
                    ...
                    pass
        print(f"  init phase: lin={init_lin} ang={init_ang} dur={init_dur}s done")

    if INIT_PHASES:
        print("=== VIO INIT COMPLETE - starting navigation ===\n")
    ...
    while sim_time < args.duration:
        ...
        # Clamp angular velocity (exp 25 fix): aggressive yaw rate from Nav2
        # recovery breaks RGBD SLAM feature tracking. Cap to 0.3 rad/s.
        _MAX_ANG_VEL = 0.3
        if ang_z > _MAX_ANG_VEL:
            ang_z = _MAX_ANG_VEL
        elif ang_z < -_MAX_ANG_VEL:
            ang_z = -_MAX_ANG_VEL
