...
EOF
chmod +x /tmp/exp51v2imu_tf_gt.sh
nohup bash /tmp/exp51v2imu_tf_gt.sh > $V2/logs/tf_warmup.log 2>&1 &
TF=$!; disown $TF
echo "TF(GT): $TF"
...
echo "=== PHASE 2: Switch to VIO+encoder + start Nav2 planner + PP follower ==="
kill $TF 2>/dev/null; sleep 2

cat > /tmp/exp51v2imu_tf_slam.sh <<'EOF'
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=85
...
EOF
chmod +x /tmp/exp51v2imu_tf_slam.sh
nohup bash /tmp/exp51v2imu_tf_slam.sh > $V2/logs/tf_slam.log 2>&1 &
TF=$!; disown $TF
echo "TF(SLAM): $TF"
sleep 3
...
cat > /tmp/exp51v2imu_nav2.sh <<EOF
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=85
...
EOF
chmod +x /tmp/exp51v2imu_nav2.sh
nohup bash /tmp/exp51v2imu_nav2.sh > $V2/logs/nav2.log 2>&1 &
NAV2=$!; disown $NAV2
echo "Nav2 planner: $NAV2"

cat > /tmp/exp51v2imu_pp.sh <<EOF
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=85
...
EOF
chmod +x /tmp/exp51v2imu_pp.sh
nohup bash /tmp/exp51v2imu_pp.sh > $V2/logs/pp_follower.log 2>&1 &
PP=$!; disown $PP
echo "PP follower: $PP"
...
sleep 3
cat > /tmp/exp51v2imu_goals.sh <<EOF
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=85
...
EOF
chmod +x /tmp/exp51v2imu_goals.sh
nohup bash /tmp/exp51v2imu_goals.sh > $V2/logs/goals.log 2>&1 &
GOALS=$!; disown $GOALS
echo "Goals: $GOALS"
