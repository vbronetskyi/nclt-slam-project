...
# PHASE 1: Isaac + warmup (pure pursuit follows south route, VIO builds map)
...
setsid /opt/isaac-sim-6.0.0/python.sh $SCRIPTS/run_husky_forest.py \
    ...
    </dev/null > $EXP/logs/isaac.log 2>&1 &
ISAAC=$!; disown $ISAAC
echo "Isaac: $ISAAC"
...
setsid ./Examples/RGB-D-Inertial/rgbd_inertial_live \
    ...
    </dev/null > $EXP/logs/vio.log 2>&1 &
VIO=$!; disown $VIO
echo "VIO: $VIO"

# GT TF for warmup
cat > /tmp/exp51_tf_gt.sh <<'EOF'
#!/bin/bash
...
EOF
chmod +x /tmp/exp51_tf_gt.sh
nohup bash /tmp/exp51_tf_gt.sh > $EXP/logs/tf_warmup.log 2>&1 &
TF=$!; disown $TF
echo "TF(GT): $TF"
...
# PHASE 2: Switch to VIO TF + disable pure pursuit + start Nav2 planner + pure pursuit follower
echo "=== PHASE 2: Switch to VIO, start Nav2 planner + PP follower ==="
kill $TF 2>/dev/null; sleep 2

cat > /tmp/exp51_tf_slam.sh <<'EOF'
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=85
...
EOF
chmod +x /tmp/exp51_tf_slam.sh
nohup bash /tmp/exp51_tf_slam.sh > $EXP/logs/tf_slam.log 2>&1 &
TF=$!; disown $TF
echo "TF(SLAM): $TF"
sleep 3

# Disable Isaac pure pursuit
touch /tmp/isaac_clear_route.txt
echo "Pure pursuit DISABLED in Isaac"
sleep 2

# Start Nav2 planner-only (map_server + planner_server + lifecycle_manager)
cat > /tmp/exp51_nav2.sh <<'EOF'
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=85
export LD_LIBRARY_PATH=${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec ros2 launch /workspace/simulation/isaac/experiments/51_hybrid_nav2_pp/config/nav2_launch_hybrid.py
EOF
chmod +x /tmp/exp51_nav2.sh
nohup bash /tmp/exp51_nav2.sh > $EXP/logs/nav2.log 2>&1 &
NAV2=$!; disown $NAV2
echo "Nav2 planner: $NAV2"

# Start pure pursuit path follower (subscribes /plan, publishes /cmd_vel)
cat > /tmp/exp51_pp.sh <<'EOF'
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=85
export LD_LIBRARY_PATH=${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec python3 /workspace/simulation/isaac/experiments/51_hybrid_nav2_pp/scripts/pure_pursuit_path_follower.py
EOF
chmod +x /tmp/exp51_pp.sh
nohup bash /tmp/exp51_pp.sh > $EXP/logs/pp_follower.log 2>&1 &
PP=$!; disown $PP
echo "PP follower: $PP"

# Wait for Nav2 planner active
for i in $(seq 1 60); do
    grep -q "Managed nodes are active\|planner_server.*active" $EXP/logs/nav2.log 2>/dev/null && break
    sleep 2
done
...
export ROS_DOMAIN_ID=85
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec python3 $EXP/scripts/send_goals_hybrid.py --trajectory /workspace/simulation/isaac/experiments/48_vio_roundtrip/logs/exp48_vio_traj.csv --spacing 4.0 --goal-timeout 300 --tolerance 1.5
EOF
chmod +x /tmp/exp51_goals.sh
nohup bash /tmp/exp51_goals.sh > $EXP/logs/goals.log 2>&1 &
GOALS=$!; disown $GOALS
echo "Goals: $GOALS"
...
echo "  tail -f $EXP/logs/pp_follower.log"
