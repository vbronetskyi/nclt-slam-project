...
SCALE_MIN = 0.5
SCALE_MAX = 2.0
SCALE_MIN_VIO_DIST = 0.5     # below this, no update (robot barely moving)
SCALE_MIN_SAMPLES = 20       # need at least N samples in each history
SCALE_ENC_STEP_MAX = 0.5     # ignore encoder teleports (> this per tick)


class RollingScaleCorrector:
    """Compare encoder-distance vs VIO-distance over rolling window.

    Apply the resulting scale to VIO displacement from a fixed alignment
    origin so the corrected position tracks real-world distances while
    keeping VIO's direction accuracy."""
    def __init__(self):
        self.vio_history = []     # [(t, x, y)]
        self.enc_history = []     # [(t, cumulative_dist)]
    ...
    def update_vio(self, t, x, y):
        self.vio_history.append((t, x, y))
        cutoff = t - SCALE_WINDOW_S
        while self.vio_history and self.vio_history[0][0] < cutoff:
    ...
    def update_encoder(self, t, enc_x, enc_y):
        if self.prev_enc_x is not None:
            ...
            if step < SCALE_ENC_STEP_MAX:
                self.enc_cumulative += step
        self.prev_enc_x = enc_x
        self.prev_enc_y = enc_y
        self.enc_history.append((t, self.enc_cumulative))
        cutoff = t - SCALE_WINDOW_S
        while self.enc_history and self.enc_history[0][0] < cutoff:
            self.enc_history.pop(0)

    def get_scale(self):
        if len(self.vio_history) < SCALE_MIN_SAMPLES or len(self.enc_history) < SCALE_MIN_SAMPLES:
            return self.scale
        vio_dist = 0.0
        for i in range(1, len(self.vio_history)):
            dx = self.vio_history[i][1] - self.vio_history[i - 1][1]
            dy = self.vio_history[i][2] - self.vio_history[i - 1][2]
            vio_dist += math.hypot(dx, dy)
        enc_dist = self.enc_history[-1][1] - self.enc_history[0][1]
        if vio_dist < SCALE_MIN_VIO_DIST:
            return self.scale
        raw = enc_dist / vio_dist
        raw = max(SCALE_MIN, min(SCALE_MAX, raw))
        self.scale = self.scale * (1 - SCALE_ALPHA) + raw * SCALE_ALPHA
        return self.scale

    def correct(self, slam_x, slam_y):
        if self.origin_x is None:
            return slam_x, slam_y
        s = self.scale
        cx = self.origin_x + (slam_x - self.origin_x) * s
        cy = self.origin_y + (slam_y - self.origin_y) * s
        return cx, cy
...
class TFRelay(Node):
    def __init__(self, use_gt=False, slam_frame=False, encoder_imu=False, slam_encoder=False):
        ...
        self.scale_corrector = RollingScaleCorrector()  # exp 51 v5
    ...
    def _slam_se3_to_nav(self, sx, sy, sz, sqx, sqy, sqz, sqw):
        ...
        if self.T_nav_slam is None:
            ...
            self.get_logger().info(
                f'SE(3)->SE(2) aligned: spawn=({self.last_x:.1f},{self.last_y:.1f}), yaw={math.degrees(gt_yaw):.1f}°')
            return self.last_x, self.last_y, gt_yaw
    ...
    def _tick_slam_encoder(self, now, x, y, z, qx, qy, qz, qw):
        ...
        if slam_ok:
            ...
            # Rolling scale correction - compare VIO vs encoder distance
            t_now = pytime.time()
            if self.scale_corrector.origin_x is None:
                self.scale_corrector.set_origin(slam_nx, slam_ny)
            self.scale_corrector.update_vio(t_now, slam_nx, slam_ny)
            self.scale_corrector.update_encoder(t_now, self.enc_x, self.enc_y)
            self.scale_corrector.get_scale()  # updates self.scale_corrector.scale
            slam_nx, slam_ny = self.scale_corrector.correct(slam_nx, slam_ny)

        if slam_ok:
            # Trust SLAM heavily: 95% corrected SLAM + 5% encoder
            SLAM_ALPHA = 0.95
        ...
        if self.log_counter % 100 == 0:
            ...
            self.get_logger().info(
                ...
                f'dist={self.enc_total_dist:.0f}m scale={self.scale_corrector.scale:.3f} '
