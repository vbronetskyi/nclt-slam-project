...
class ScaleCorrector:
    """Rolling-window scale correction. Compares encoder distance vs VIO distance
    and applies a smoothed scale factor to VIO deltas."""
    def __init__(self, window_s=SCALE_WINDOW_S, alpha=SCALE_ALPHA,
                 min_vio_dist=SCALE_MIN_VIO_DIST, s_min=SCALE_MIN, s_max=SCALE_MAX):
    ...
    def get_scale(self):
        ...
        self.current_scale = max(self.s_min, min(self.s_max, self.current_scale))
        return self.current_scale
...
class TFRelay(Node):
    def __init__(self, use_gt=False, slam_frame=False, encoder_imu=False, slam_encoder=False):
        ...
        self.slam_corr_y = None
        self.prev_slam_nav_x = None
        self.prev_slam_nav_y = None
        self.current_scale = 1.0
    ...
    def _tick_slam_encoder(self, now, x, y, z, qx, qy, qz, qw):
        ...
        if slam_ok:
            ...
            self.current_scale = self.scale_corrector.get_scale()
            if self.slam_corr_x is None:
                self.slam_corr_x = slam_nx
                self.slam_corr_y = slam_ny
            else:
                dx = slam_nx - self.prev_slam_nav_x
                dy = slam_ny - self.prev_slam_nav_y
                self.slam_corr_x += dx * self.current_scale
                self.slam_corr_y += dy * self.current_scale
            self.prev_slam_nav_x = slam_nx
            self.prev_slam_nav_y = slam_ny
            slam_nx = self.slam_corr_x
            slam_ny = self.slam_corr_y
        ...
        if self.log_counter % 100 == 0:
            ...
            self.get_logger().info(
                ...
                f'dist={self.enc_total_dist:.0f}m scale={self.current_scale:.3f} '
