...
parser.add_argument("--route", type=str, default=None,
                    choices=["road", "north", "south", "road_nav2", "warmup"],
                    help="predefined route: road (S-curve), north (forest), south (forest), warmup (straight)")
...
if _shrub_root.IsValid():
    ...
    for _c in _shrub_root.GetChildren():
        if _src:
            ...
            _vis.AddTranslateOp().Set(Gf.Vec3d(_t[0], _t[1], _t[2]))
            _vis.GetPrim().GetReferences().AddInternalReference(_src)
            _n_vis += 1
    print(f"  shrubs: {_n_vis} visuals added @ 0.25x scale (src={_src})")
...
_husky_translate_op.Set(Gf.Vec3d(_spawn_x, _spawn_y, 0.2))  # updated after terrain_height defined
...
# Both get Phidgets 1042 spec noise (0.02 m/s², 0.005 rad/s) added.

def _compute_synth_imu(pos_arg, quat_xyzw, pos_for_vel, t):
    """Synthetic IMU: PhysX velocity for accel + quaternion-diff for gyro."""
    global _synth_prev_vel_world, _synth_prev_quat, _synth_prev_time, _synth_prev_omega
    from scipy.spatial.transform import Rotation as _R

    quat = np.asarray(quat_xyzw, dtype=float)
    vel_world = np.asarray(_husky_rigid.get_linear_velocity(), dtype=float)
    ...
    # Accel: world-frame velocity diff, LPF, +gravity, body-frame
    ...
    # Gyro: body-frame angular velocity from quaternion diff (clean - avoids
    # PhysX angular velocity jitter observed in first v7 attempt)
    R_prev = _R.from_quat(_synth_prev_quat)
    omega_body = (R_prev.inv() * R_now).as_rotvec() / dt
    omega_body = 0.4 * omega_body + 0.6 * _synth_prev_omega  # small causal smoothing
...
try:
    ...
    while sim_time < args.duration:
        ...
        if _recording and frame_n % 1 == 0:
            ...
            # wheel odometry (logical frame = 20 steps × 1/200s = 0.1s)
            dt = _step_dt * 20
