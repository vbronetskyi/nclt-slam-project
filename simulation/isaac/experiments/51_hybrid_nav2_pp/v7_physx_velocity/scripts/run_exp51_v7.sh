...
V7=$EXP/v7_physx_velocity
SCRIPTS=/workspace/simulation/isaac/scripts
SLAM=/workspace/third_party/ORB_SLAM3
DOMAIN=${ROS_DOMAIN_ID:-85}

cp $V7/config/south_roundtrip_route.json /workspace/simulation/isaac/route_memory/south/anchors.json
python3 -c "
import json
...
setsid /opt/isaac-sim-6.0.0/python.sh $SCRIPTS/run_husky_forest.py \
    ...
    </dev/null > $V7/logs/isaac.log 2>&1 &
ISAAC=$!; disown $ISAAC
echo "Isaac: $ISAAC"
...
echo "REC=$REC" > $V7/logs/run_info.txt

cd $SLAM
setsid ./Examples/RGB-D-Inertial/rgbd_inertial_live \
    Vocabulary/ORBvoc.txt $V7/config/vio_th160.yaml $REC \
    </dev/null > $V7/logs/vio.log 2>&1 &
VIO=$!; disown $VIO
echo "VIO: $VIO"

cat > /tmp/exp51v7_tf_gt.sh <<'EOF'
#!/bin/bash
...
EOF
chmod +x /tmp/exp51v7_tf_gt.sh
nohup bash /tmp/exp51v7_tf_gt.sh > $V7/logs/tf_warmup.log 2>&1 &
TF=$!; disown $TF
echo "TF(GT): $TF"
...
cat > /tmp/exp51v7_tf_slam.sh <<'EOF'
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=85
...
EOF
chmod +x /tmp/exp51v7_tf_slam.sh
nohup bash /tmp/exp51v7_tf_slam.sh > $V7/logs/tf_slam.log 2>&1 &
TF=$!; disown $TF
...
cat > /tmp/exp51v7_nav2.sh <<EOF
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=85
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec ros2 launch $V7/config/nav2_launch_hybrid.py
EOF
chmod +x /tmp/exp51v7_nav2.sh
nohup bash /tmp/exp51v7_nav2.sh > $V7/logs/nav2.log 2>&1 &
NAV2=$!; disown $NAV2
echo "Nav2: $NAV2"

cat > /tmp/exp51v7_pp.sh <<EOF
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=85
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec python3 $V7/scripts/pure_pursuit_path_follower.py
EOF
chmod +x /tmp/exp51v7_pp.sh
nohup bash /tmp/exp51v7_pp.sh > $V7/logs/pp_follower.log 2>&1 &
PP=$!; disown $PP
echo "PP: $PP"

for i in $(seq 1 60); do
    grep -q "Managed nodes are active\|planner_server.*active" $V7/logs/nav2.log 2>/dev/null && break
    sleep 2
done
echo "Nav2 ready"
...
sleep 3
cat > /tmp/exp51v7_goals.sh <<EOF
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=85
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec python3 $V7/scripts/send_goals_hybrid.py --trajectory /workspace/simulation/isaac/experiments/48_vio_roundtrip/logs/exp48_vio_traj.csv --spacing 4.0 --goal-timeout 300 --tolerance 1.5
EOF
chmod +x /tmp/exp51v7_goals.sh
nohup bash /tmp/exp51v7_goals.sh > $V7/logs/goals.log 2>&1 &
GOALS=$!; disown $GOALS
echo "Goals: $GOALS"
...
echo "Monitor:"
echo "  tail -f $V7/logs/tf_slam.log"
echo "  grep GUARD $V7/logs/tf_slam.log"
