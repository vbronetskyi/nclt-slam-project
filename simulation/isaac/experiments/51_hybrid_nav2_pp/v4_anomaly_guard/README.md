...
seconds during each anomaly should let the encoder carry thorugh the bad
...
- Robot drove 909 m (route is +-300 m) cascading wildly off course. Final err
  293 m at gt=(43.5, 391.3) - just no localization.
...
- `logs/` - full run logs; `grep GUARD tf_slam.log` shows activations
  (0 in this succesful run; 836 in the failed first-attempt run that
  logs above)
