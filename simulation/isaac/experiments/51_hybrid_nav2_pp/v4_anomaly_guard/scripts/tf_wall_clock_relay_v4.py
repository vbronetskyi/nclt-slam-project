...
SLAM_GUARD_MAX_ACTIVATIONS = 20     # beyond this stop freezing - clearly broken


class SlamAnomalyGuard:
    """Detect and reject single-tick SLAM position jumps > SLAM_GUARD_MAX_JUMP_M.

    check() returns (x, y, yaw, is_anomaly). During freeze the last
    known-good pose is returned. Prev-pose is reset when freeze ends so
    the post-freeze SLAM delta doesn't itself look anomalous."""
    def __init__(self):
        self.prev_x = None
        self.prev_y = None
    ...
    def check(self, x, y, yaw, t):
        ...
        # Freeze active: ignore SLAM input
        if t < self.frozen_until:
            return self.last_good_x, self.last_good_y, self.last_good_yaw, True

        # Freeze just ended - resync prev to current so next delta is small
        if self.prev_x is None or (t - self.init_t) < SLAM_GUARD_WARMUP_S:
            self.prev_x, self.prev_y = x, y
            self.last_good_x, self.last_good_y, self.last_good_yaw = x, y, yaw
            return x, y, yaw, False
        ...
        # Single-tick displacement check
        jump = math.hypot(x - self.prev_x, y - self.prev_y)
        if jump > SLAM_GUARD_MAX_JUMP_M:
            self.frozen_until = t + SLAM_GUARD_FREEZE_S
...
class TFRelay(Node):
    def __init__(self, use_gt=False, slam_frame=False, encoder_imu=False, slam_encoder=False):
        ...
        self.slam_guard = SlamAnomalyGuard()  # exp 51 v4
    ...
    def _tick_slam_encoder(self, now, x, y, z, qx, qy, qz, qw):
        ...
        if slam_ok:
            ...
            if is_anomaly and self.slam_guard.activation_count == 1:
                # Log on first activation (subsequent ticks during freeze stay quiet)
                pass  # noop here; detailed log below
            # Trust SLAM over encoder - VIO is calibrated, encoder drifts easily

        if slam_ok:
            if is_anomaly:
                # During anomaly freeze: encoder carries position, VIO yaw still useful
                nav_x = self.enc_x
                nav_y = self.enc_y
                nav_yaw = self.enc_yaw
                self.using_slam = False
            else:
                # Normal: 95% SLAM + 5% encoder
                SLAM_ALPHA = 0.95
        ...
        if self.slam_guard.activation_count > self._last_guard_count:
            self._last_guard_count = self.slam_guard.activation_count
            self.get_logger().warn(
                f'[GUARD] SLAM anomaly #{self._last_guard_count} at '
                f'gt=({x:.1f},{y:.1f}) dist={self.enc_total_dist:.0f}m - '
                f'freezing SLAM for {SLAM_GUARD_FREEZE_S}s')
