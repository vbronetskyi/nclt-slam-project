...
E57=/workspace/simulation/isaac/experiments/57_depth_correlation
SCRIPTS=/workspace/simulation/isaac/scripts
SLAM=/workspace/third_party/ORB_SLAM3
DOMAIN=${ROS_DOMAIN_ID:-85}

TEACH=$E57/teach
OUT=${REPEAT_OUT_DIR:-$E57/results/repeat_run}
...
import re
for p, pat in [
    ('$E57/config/nav2_planner_only.yaml', r'yaml_filename:.*'),
    ('$E57/config/nav2_launch_hybrid.py',  r'(map_yaml|config)\s*=\s*"[^"]*"'),
]:
    txt = open(p).read()
    if 'map_yaml' in pat:
        txt = re.sub(r'map_yaml = "[^"]*"', f'map_yaml = "$TEACH_MAP"', txt, count=1)
        txt = re.sub(r'config = "[^"]*"', f'config = "$E57/config/nav2_planner_only.yaml"', txt, count=1)
    else:
...
cp $E57/config/south_roundtrip_route.json /workspace/simulation/isaac/route_memory/south/anchors.json
python3 -c "
import json
...
pkill -9 -f "run_husky_forest|rgbd_inertial|tf_wall_clock|planner_server|map_server|pure_pursuit_path|send_goals_hybrid|lifecycle_manager|costmap_snapshotter|turnaround_supervisor|plan_logger|visual_landmark|depth_correlation_matcher|ros2 launch" 2>/dev/null || true
sleep 5
...
setsid ./Examples/RGB-D-Inertial/rgbd_inertial_live \
    Vocabulary/ORBvoc.txt $E57/config/vio_th160.yaml $REC \
...
export ROS_DOMAIN_ID=85
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec python3 $E57/scripts/tf_wall_clock_relay_v55.py --slam-encoder
EOF
...
cat > /tmp/exp57r_matcher.sh <<EOF
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=85
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec python3 $E57/scripts/visual_landmark_matcher.py \
    --landmarks $LANDMARKS --out-csv $OUT/anchor_matches.csv
EOF
chmod +x /tmp/exp57r_matcher.sh
...
export ROS_DOMAIN_ID=85
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec python3 $E57/scripts/depth_correlation_matcher.py \
    --teach-map $TEACH_MAP --out-csv $OUT/depth_icp_matches.csv
EOF
chmod +x /tmp/exp57r_icp.sh
nohup bash /tmp/exp57r_icp.sh > $OUT/depth_correlation_matcher.log 2>&1 &
ICP=$!; disown $ICP; echo "DepthICPMatcher: $ICP"
...
export ROS_DOMAIN_ID=85
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec ros2 launch $E57/config/nav2_launch_hybrid.py
EOF
...
export ROS_DOMAIN_ID=85
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec python3 $E57/scripts/pure_pursuit_path_follower.py
EOF
...
export ROS_DOMAIN_ID=85
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec python3 $E57/scripts/turnaround_supervisor.py --turnaround-x 60.0 --past-margin 2.0
EOF
chmod +x /tmp/exp55r_sup.sh
nohup bash /tmp/exp55r_sup.sh > $OUT/supervisor.log 2>&1 &
SUP=$!; disown $SUP; echo "Supervisor: $SUP"

cat > /tmp/exp55r_cms.sh <<EOF
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=85
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec python3 $E57/scripts/costmap_snapshotter.py --out-dir $OUT/snapshots --period 5.0 --topic /global_costmap/costmap
EOF
chmod +x /tmp/exp55r_cms.sh
nohup bash /tmp/exp55r_cms.sh > $OUT/costmap_snapshotter.log 2>&1 &
...
export ROS_DOMAIN_ID=85
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec python3 $E57/scripts/plan_logger.py --out-dir $OUT/plans
EOF
...
export ROS_DOMAIN_ID=85
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec python3 $E57/scripts/send_goals_hybrid.py --trajectory /workspace/simulation/isaac/experiments/48_vio_roundtrip__/logs/exp48_vio_traj.csv --spacing 4.0 --goal-timeout 120 --tolerance 3.0
EOF
