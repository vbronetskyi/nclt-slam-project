...
class VisualLandmarkMatcher(Node):
    def __init__(self, pkl_path, log_csv, return_pkl=None, swap_flag='/tmp/matcher_swap_return.txt'):
        super().__init__('visual_landmark_matcher')
        self.pkl_path = pkl_path
        self.return_pkl = return_pkl
    ...
    def _maybe_swap_to_return(self):
        """If swap-flag exists and we have a return pkl, replace landmark set."""
        if (self._swapped or self.return_pkl is None
                or not os.path.exists(self.swap_flag)):
            return
        with open(self.return_pkl, 'rb') as f:
            data = pickle.load(f)
        self.pkl_path = self.return_pkl
        self.pkl_data = data
        self.landmarks = data['landmarks']
        self.xy = np.array([[lm['pose'][0], lm['pose'][1]] for lm in self.landmarks])
        self.heading = np.array([self._lm_heading_rad(lm) for lm in self.landmarks])
        self.n_initial_landmarks = len(self.landmarks)
        self.n_accumulated = 0   # fresh accumulation on return leg
        self._swapped = True
        self.get_logger().warn(
            f'[SWAP] reloaded return-leg landmarks ({len(self.landmarks)}) from '
            f'{self.return_pkl} - outbound set dropped')

    def _tick(self):
        self._maybe_swap_to_return()
...
def main():
    ...
    ap.add_argument('--landmarks', required=True,
                    help='teach landmarks.pkl (outbound subset for exp 69)')
    ap.add_argument('--landmarks-return', default=None,
                    help='optional return-leg landmarks.pkl; loaded when swap-flag appears')
    ap.add_argument('--swap-flag', default='/tmp/matcher_swap_return.txt',
                    help='path to flag file that triggers swap to return-leg set')
    ap.add_argument('--out-csv', required=True)
    args = ap.parse_args()

    rclpy.init()
    node = VisualLandmarkMatcher(args.landmarks, args.out_csv,
                                 return_pkl=args.landmarks_return,
