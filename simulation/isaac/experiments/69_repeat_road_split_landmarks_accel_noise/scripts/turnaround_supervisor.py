...
class TurnaroundSupervisor(Node):
    ...
    def fire(self):
        self.fired = True
        now = str(time.time())
        # (1) Isaac drops cones+tent
        with open("/tmp/isaac_remove_obstacles.txt", "w") as f: f.write(now)
        # (2) Matcher swaps to return-leg landmarks (exp 69)
        with open("/tmp/matcher_swap_return.txt", "w") as f: f.write(now)
        self.get_logger().warn(
            "[FIRE] wrote /tmp/isaac_remove_obstacles.txt (Isaac drops cones+tent) "
            "+ /tmp/matcher_swap_return.txt (matcher reloads return-leg anchors). "
            "Nav2 obstacle_layer self-clears via clearing=True.")
