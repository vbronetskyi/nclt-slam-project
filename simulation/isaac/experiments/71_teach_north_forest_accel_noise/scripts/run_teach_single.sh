...
case "$ROUTE" in
    ...
    north_forest) ORIG_X=-110; ORIG_Y=-15; W=200; H=55 ;;
...
EOF
chmod +x /tmp/exp71_tf.sh
nohup bash /tmp/exp71_tf.sh > $OUT/tf_slam.log 2>&1 &
...
EOF
chmod +x /tmp/exp71_drift.sh
nohup bash /tmp/exp71_drift.sh > $OUT/drift_monitor.log 2>&1 &
DRIFT=$!; disown $DRIFT; echo "DriftMonitor: $DRIFT"

cat > /tmp/exp71_depth.sh <<EOF
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=$DOMAIN
...
EOF
chmod +x /tmp/exp71_depth.sh
nohup bash /tmp/exp71_depth.sh > $OUT/teach_depth_mapper.log 2>&1 &
DEPTH=$!; disown $DEPTH; echo "DepthMapper: $DEPTH"

cat > /tmp/exp71_lm.sh <<EOF
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=$DOMAIN
...
EOF
chmod +x /tmp/exp71_lm.sh
nohup bash /tmp/exp71_lm.sh > $OUT/landmark_recorder.log 2>&1 &
