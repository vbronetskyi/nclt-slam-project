...
parser.add_argument("--use-hybrid", action="store_true",
                    help="hybrid navigator: gap nav + A* planner")
...
_exp_dir = ("39_hybrid_navigator" if args.use_hybrid
            else "38_fast_planner" if args.use_planner
...
if args.use_hybrid:
    ...
    print("  HybridNavigator (gap nav + A*) enabled")
    ...
    while sim_time < args.duration and not follower.finished:
        ...
        if frame_count > 60 and frame_count % DEPTH_EVERY == 0:
            ...
            if depth_raw is not None:
                ...
                if hybrid_nav is not None:
                    # Hybrid mode: gap nav on open road, A* near obstacles
                    target_dx = _gt_tx - rx
                    target_dy = _gt_ty - ry
                    last_nav_lin, last_nav_ang, last_nav_mode, last_nav_debug = \
                        hybrid_nav.update(
                            rx, ry, ryaw, depth_img,
                            heading_err, target_dx, target_dy)
        ...
        if int(sim_time) % 10 == 0 and abs(sim_time - int(sim_time)) < dt:
            ...
            print(
                ...
                + (f" center={d.get('center_dist','?')}m"
                   if hybrid_nav and 'center_dist' in d else ""))
