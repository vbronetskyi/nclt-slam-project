...
class TFRelay(Node):
    def __init__(self, use_gt=False, slam_frame=False):
        ...
        # SLAM stale detection + odom-fallback dead reckoning (exp 32)
        self.stale_last_check_time = pytime.time()
        self.stale_last_check_slam_nx = 0.0
        self.stale_last_check_slam_ny = 0.0
        self.stale_odom_accum = 0.0      # cmd_vel linear integrated since last check
        self.slam_is_stale = False
        self.stale_start_fused_x = 0.0
        self.stale_start_fused_y = 0.0
        self.stale_start_fused_yaw = 0.0
        self.stale_start_slam_nx = 0.0
        self.stale_start_slam_ny = 0.0
    ...
    def _tick_slam_frame(self, now, x, y, z, qx, qy, qz, qw):
        ...
        if not self.filter_initialized:
            ...
            self.stale_last_check_slam_nx = nav_x
            self.stale_last_check_slam_ny = nav_y
        ...
        if now_wall - self.stale_last_check_time >= STALE_CHECK_INTERVAL:
            slam_moved = math.hypot(
                nav_x - self.stale_last_check_slam_nx,
                nav_y - self.stale_last_check_slam_ny)
            if not self.slam_is_stale:
                if (self.stale_odom_accum > STALE_ODOM_THRESHOLD
                        and slam_moved < STALE_SLAM_MIN_MOVE):
                    self.slam_is_stale = True
                    self.stale_event_count += 1
                    self.stale_start_fused_x = self.fused_x
                    self.stale_start_fused_y = self.fused_y
                    self.stale_start_fused_yaw = self.fused_yaw
                    self.stale_start_slam_nx = nav_x
                    self.stale_start_slam_ny = nav_y
                    self.get_logger().warn(
                        f'SLAM STALE #{self.stale_event_count}: '
                        f'odom={self.stale_odom_accum:.2f}m slam_moved={slam_moved:.3f}m '
                        f'- switching to odom dead-reckon from '
                        f'({self.fused_x:.1f},{self.fused_y:.1f})')
            else:
                slam_total_move = math.hypot(
                    nav_x - self.stale_start_slam_nx,
                    nav_y - self.stale_start_slam_ny)
                if slam_total_move > STALE_RECOVERY_MOVE:
                    self.slam_is_stale = False
                    self.get_logger().info(
                        f'SLAM RECOVERED: moved {slam_total_move:.2f}m from stale point')
            self.stale_last_check_slam_nx = nav_x
            self.stale_last_check_slam_ny = nav_y
            self.stale_odom_accum = 0.0
            self.stale_last_check_time = now_wall
