...
usage:
  ...
  python3 tf_wall_clock_relay.py [--use-gt] [--encoder-imu] [--slam-frame]
"""
import math
import re
import sys
...
class TFRelay(Node):
    def __init__(self, use_gt=False, slam_frame=False, encoder_imu=False, slam_encoder=False):
        ...
        self.anchor_strong_streak = 0   # consecutive strong matches (hysteresis)
        self.ANCHOR_STALE_S = 3.0
        self.ANCHOR_STRONG_STD = 0.1
        self.ANCHOR_OK_STD = 0.2
        self.ANCHOR_HYSTERESIS_N = 2
    ...
    def _tick_gt(self, now, x, y, z, qx, qy, qz, qw):
        """GT mode: map->odom=identity, odom->base_link=GT pose."""
        t1 = TransformStamped()
        t1.header.stamp = now
    ...
    def _tick_slam_encoder(self, now, x, y, z, qx, qy, qz, qw):
        ...
        if slam_ok:
            ...
            if self.anchor_last is not None:
                ...
                # v62 A3 anchor sanity: reject the anchor if it suggests only
                # a tiny correction while SLAM and encoder disagree by a lot
                # - that pattern indicates the matcher latched onto a wrong
                # teach landmark (already-drifted self-confirmation).
                anchor_shift = math.hypot(self.anchor_last[1] - slam_nx,
                                          self.anchor_last[2] - slam_ny)
                slam_enc_disagree = math.hypot(slam_nx - self.enc_x,
                                               slam_ny - self.enc_y)
                anchor_insane = (slam_enc_disagree > 2.0 and
                                 anchor_shift < 0.5 and
                                 std < 0.15)
                if anchor_insane:
                    # Skip this anchor: keep no_anchor regime, log once.
                    if not getattr(self, '_sanity_logged_ts', None) or \
                       pytime.time() - self._sanity_logged_ts > 5.0:
                        self.get_logger().warn(
                            f'SANITY reject anchor: enc_disagree='
                            f'{slam_enc_disagree:.1f}m anchor_shift='
                            f'{anchor_shift:.2f}m std={std:.2f}')
                        self._sanity_logged_ts = pytime.time()
                elif staleness <= self.ANCHOR_STALE_S and std <= self.ANCHOR_OK_STD:
                    anchor_x_use = self.anchor_last[1]
                    anchor_y_use = self.anchor_last[2]
                    # v63 HARD JUMP: if anchor says we're > 3 m from current
                    # fused pose AND its std is very low, trust it outright.
                    # This is the kidnapped-robot recovery case - relocation
                    # matcher found a confident match far from VIO belief.
                    if std <= 0.08 and anchor_shift > 3.0:
                        regime = 'jump'
                    elif (std <= self.ANCHOR_STRONG_STD
                            and self.anchor_strong_streak >= self.ANCHOR_HYSTERESIS_N):
                        regime = 'strong'
                    else:
                        regime = 'ok'
            if regime == 'jump':
                # Hard reset fused pose to anchor; encoder bias also cleared.
                nav_x = anchor_x_use
                nav_y = anchor_y_use
                self.enc_x = anchor_x_use
                self.enc_y = anchor_y_use
                if not getattr(self, '_jump_logged_ts', None) or \
                   pytime.time() - self._jump_logged_ts > 5.0:
                    self.get_logger().warn(
                        f'JUMP applied: nav->({nav_x:.1f},{nav_y:.1f}) '
                        f'from ({slam_nx:.1f},{slam_ny:.1f}) std={std:.2f}')
                    self._jump_logged_ts = pytime.time()
            elif regime == 'strong':
        ...
        # v63r2: publish SLAM-vs-encoder disagreement as file-based signal.
        # The visual matcher uses this to gate global relocalisation so
        # reloc only fires when there's actually a drift disagreement.
        if slam_ok:
            slam_enc_d_est = math.hypot(slam_nx - self.enc_x,
                                        slam_ny - self.enc_y)
        else:
            slam_enc_d_est = 0.0
        try:
            with open('/tmp/drift_est.txt', 'w') as _df:
                _df.write(f'{slam_enc_d_est:.3f}\n')
        except Exception:
            pass
