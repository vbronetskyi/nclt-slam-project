...
class HybridGoalSender(Node):
    def __init__(self, waypoints, goal_timeout=300.0, tolerance=1.5):
        ...
        # v60: direct /cmd_vel publish for clean STOP after RESULT
        from geometry_msgs.msg import Twist as _Twist
        self._Twist = _Twist
        self.cmd_pub = self.create_publisher(_Twist, '/cmd_vel', 10)
    ...
    def _read_gt_pose(self):
        """Sim-only GT readout (for critical-endpoint precise finisher)."""
        try:
            with open("/tmp/isaac_pose.txt") as f:
                parts = f.readline().split()
            x = float(parts[0]); y = float(parts[1])
            qx = float(parts[3]); qy = float(parts[4])
            qz = float(parts[5]); qw = float(parts[6])
    ...
    def _precise_approach(self, tx, ty, label, budget=60.0, tol=0.5):
        """GT-based DR finisher for turnaround + end. Publishes empty Path
        ...
        for _ in range(3):
            self.plan_pub.publish(empty); time.sleep(0.05)
        self.get_logger().info(
            f'  PRECISE APPROACH -> {label} ({tx:.2f},{ty:.2f})')
        t0 = time.time()
        stop_twist = self._Twist()
        d = 999
        while time.time() - t0 < budget:
            gx, gy, gyaw = self._read_gt_pose()
            if gx is None:
            ...
            if d < tol:
                for _ in range(3):
                    self.cmd_pub.publish(stop_twist); time.sleep(0.05)
                self.get_logger().info(
                    f'  PRECISE ARRIVED {label} (d={d * 100:.1f} cm)')
                return True
            des_yaw = math.atan2(dy, dx)
            yaw_err = (des_yaw - gyaw + math.pi) % (2 * math.pi) - math.pi
            cmd = self._Twist()
            if abs(yaw_err) > 0.25:
                cmd.angular.z = 0.4 if yaw_err > 0 else -0.4
            else:
                cmd.linear.x = min(0.25, 0.4 * d)
                cmd.angular.z = max(-0.4, min(0.4, 0.8 * yaw_err))
            self.cmd_pub.publish(cmd)
            time.sleep(0.1)
        for _ in range(3):
            self.cmd_pub.publish(stop_twist); time.sleep(0.05)
        self.get_logger().warn(
            f'  PRECISE TIMEOUT {label} (d={d * 100:.0f} cm)')
        return False
    ...
    def _find_detour(self, x, y, relax_cost=False):
        """Find a safe detour point around (x,y).  Returns (cx,cy) or
        (None,None).

        Tries multiple radii (2 / 3 / 4 / 5 m - v60 starts tighter to stay
        close to the original WP), samples 24 candidates per ring, accepts
        a candidate only if (a) it is NOT within known obstacle clearance,
        and (b) its costmap cost < DETOUR_MAX_COST.

        relax_cost: used for final WP where teach-map tree inflation
        produces phantom cost-99 cells that have nothing physical behind
        them. Lets the cost ceiling rise to 80 so the robot can finish
        near the true endpoint.
        """
        best = None
        best_cost = 999
        n_samples = 24
        cost_max = 80 if relax_cost else self.DETOUR_MAX_COST
        for radius in (2.0, 2.5, 3.0, 3.5, 4.0):  # v62: tighter than exp 60's 2-5, keeps robot in teach visual corridor
            for k in range(n_samples):
                ...
                if cost < cost_max and cost < best_cost:
                    best_cost = cost
                    best = (cx, cy)
            if best is not None:
                return best
    ...
    def run(self):
        ...
        # v62: critical endpoints - turnaround + end - get precise finisher.
        ...
        for i in range(start, self.n_wps):
            ...
            is_critical = i in self._precise_targets
            if self.skip_flags[i] and not is_critical:
                self.get_logger().warn(
            ...
            if unsafe_reason is not None and not is_critical:
                dx, dy = self._find_detour(x, y, relax_cost=(i == self.n_wps - 1))
                if dx is not None:
                    self.get_logger().warn(
            ...
            if is_critical:
                label, (tx, ty) = self._precise_targets[i]
                ok = self._precise_approach(tx, ty, label)
                if not ok:
                    self.get_logger().warn(
                        f'  CRITICAL {label}: retrying precise approach')
                    ok = self._precise_approach(tx, ty, label, budget=45.0)
                if ok:
                    self.reached += 1
                else:
                    self.skipped += 1
