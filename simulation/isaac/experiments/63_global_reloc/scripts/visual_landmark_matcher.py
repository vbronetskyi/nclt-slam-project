...
RELOC_MIN_INLIERS = 18          # up from 10
RELOC_REPROJ_MAX_PX = 1.5       # tighter than 2.0
...
class VisualLandmarkMatcher(Node):
    ...
    def _tick(self):
        ...
        drift_est = 0.0
        try:
            with open('/tmp/drift_est.txt') as _df:
                drift_est = float(_df.readline().strip())
        except Exception:
            pass
        if (not cand_idx
                and (ts - self.last_anchor_ts) > RELOC_AGE_S
                and drift_est > RELOC_DRIFT_M):
            heading_mask = hdg_err < hdg_tol
            global_pool = np.where(heading_mask)[0]
            # Rank by descriptor match count (fast crossCheck); take top-N.
            scored = []
            for li in global_pool:
                desc_t = self.landmarks[li]['descriptors']
                ...
                if len(good) >= MIN_MATCHES:
                    scored.append((len(good), int(li)))
            scored.sort(reverse=True)
            cand_idx = [li for _, li in scored[:RELOC_MAX_CANDIDATES]]
            relocating = True
        ...
        for li in cand_idx:
            ...
            min_inl = RELOC_MIN_INLIERS if relocating else MIN_INLIERS
            max_err = RELOC_REPROJ_MAX_PX if relocating else REPROJ_MAX_PX
            if not ok or inliers is None or len(inliers) < min_inl:
                continue
        ...
        if not relocating and consistency_d > CONSISTENCY_M:
        ...
        if relocating:
            self.get_logger().warn(
                f'GLOBAL-RELOC jump {consistency_d:.1f} m  '
                f'vio=({vio_xy[0]:.1f},{vio_xy[1]:.1f})  '
                f'anchor=({anchor_pose[0]:.1f},{anchor_pose[1]:.1f})  '
                f'inliers={n_inliers} reproj={reproj_err:.2f}px')
