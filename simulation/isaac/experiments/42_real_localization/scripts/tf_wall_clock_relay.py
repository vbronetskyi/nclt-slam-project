...
class TFRelay(Node):
    ...
    def _read_slam_pose(self):
        """Read SLAM pose from file. Returns (nav_x, nav_y, nav_yaw, ok)."""
        try:
            ...
            # Check stale (same timestamp = no new frame processed)
            ...
            if self.slam_origin is None:
                self.slam_origin = (sx, sy, sz, slam_yaw_raw)

            s0x, s0y, s0z, s0yaw = self.slam_origin
            nav_x = SPAWN_X + (sz - s0z)
            nav_y = SPAWN_Y - (sx - s0x)
            nav_yaw = -(slam_yaw_raw - s0yaw)

            return nav_x, nav_y, nav_yaw, True
        except (FileNotFoundError, ValueError, IndexError):
            return 0, 0, 0, False

    def _tick_slam_encoder(self, now, x, y, z, qx, qy, qz, qw):
        """SLAM+Encoder mode: ORB-SLAM3 when tracking, encoder+compass fallback.

        When SLAM is tracking: use SLAM pose (camera->nav conversion)
        When SLAM is lost/stale: use encoder+compass (same as Level 2)
        """
        ...
        #Update encoder odometry (always, as fallback)
        COMPASS_NOISE = 0.05
        ...
        # Try reading SLAM pose
        slam_nx, slam_ny, slam_nyaw, slam_ok = self._read_slam_pose()

        if slam_ok:
            # Use SLAM position + compass heading (SLAM yaw is unreliable in 2D extraction)
            nav_x = slam_nx
            nav_y = slam_ny
            nav_yaw = self.enc_yaw  # compass heading is more reliable than SLAM 2D yaw
