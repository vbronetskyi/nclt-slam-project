...
class HybridGoalSender(Node):
    ...
    def _find_detour(self, x, y, relax_cost=False):
        ...
        for radius in (4.0, 5.0, 6.0, 7.0):  # v61: reverted from 2-5 (exp 60 had too many rejects)
    ...
    def _read_gt_pose(self):
        """Sim-only GT readout from /tmp/isaac_pose.txt. Used exclusively in
        the precise-approach finisher to land on an absolute point - NOT in
        normal navigation (that uses SLAM tf).
        """
        try:
    ...
    def _precise_approach(self, tx, ty, label, budget=60.0, tol=0.5):
        ...
        while time.time() - t0 < budget:
            ...
            cmd = self._Twist()
            if abs(yaw_err) > 0.25:          # rotate first
                cmd.angular.z = 0.4 if yaw_err > 0 else -0.4
            else:                             # drive with curvature
