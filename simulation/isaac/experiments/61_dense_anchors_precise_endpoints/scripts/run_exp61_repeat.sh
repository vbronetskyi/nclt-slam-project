...
E61=/workspace/simulation/isaac/experiments/61_dense_anchors_precise_endpoints
SCRIPTS=/workspace/simulation/isaac/scripts
SLAM=/workspace/third_party/ORB_SLAM3
DOMAIN=${ROS_DOMAIN_ID:-85}

TEACH=$E61/teach
OUT=${REPEAT_OUT_DIR:-$E61/results/repeat_run}
...
import re
for p, pat in [
    ('$E61/config/nav2_planner_only.yaml', r'yaml_filename:.*'),
    ('$E61/config/nav2_launch_hybrid.py',  r'(map_yaml|config)\s*=\s*"[^"]*"'),
]:
    txt = open(p).read()
    if 'map_yaml' in pat:
        txt = re.sub(r'map_yaml = "[^"]*"', f'map_yaml = "$TEACH_MAP"', txt, count=1)
        txt = re.sub(r'config = "[^"]*"', f'config = "$E61/config/nav2_planner_only.yaml"', txt, count=1)
    else:
...
cp $E61/config/south_roundtrip_route.json /workspace/simulation/isaac/route_memory/south/anchors.json
python3 -c "
import json
...
setsid ./Examples/RGB-D-Inertial/rgbd_inertial_live \
    Vocabulary/ORBvoc.txt $E61/config/vio_th160.yaml $REC \
...
export ROS_DOMAIN_ID=85
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec python3 $E61/scripts/tf_wall_clock_relay_v55.py --slam-encoder
EOF
...
export ROS_DOMAIN_ID=85
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec python3 $E61/scripts/visual_landmark_matcher.py \
...
export ROS_DOMAIN_ID=85
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec ros2 launch $E61/config/nav2_launch_hybrid.py
EOF
...
export ROS_DOMAIN_ID=85
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec python3 $E61/scripts/pure_pursuit_path_follower.py
EOF
...
export ROS_DOMAIN_ID=85
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec python3 $E61/scripts/turnaround_supervisor.py --turnaround-x 60.0 --past-margin 2.0
EOF
...
export ROS_DOMAIN_ID=85
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec python3 $E61/scripts/plan_logger.py --out-dir $OUT/plans
EOF
...
export ROS_DOMAIN_ID=85
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec python3 $E61/scripts/send_goals_hybrid.py --trajectory /workspace/simulation/isaac/experiments/48_vio_roundtrip__/logs/exp48_vio_traj.csv --spacing 4.0 --goal-timeout 300 --tolerance 3.0
EOF
