...
class PurePursuitFollower(Node):
    ...
    def path_cb(self, msg):
        if len(msg.poses) == 0:
            # v64: empty Path = STOP signal from goal_sender after RESULT
            # or before precise-finisher takeover.
            self.path = None
            self.path_done = True
            self.cmd_pub.publish(Twist())
            self.get_logger().info('Empty path received - STOP')
            return
        if len(msg.poses) < 2:
            return
        self.path = msg
    ...
    def tick(self):
        ...
        # v55 wedge recovery - reverse when robot physically wedged
        if t_now < self.wedge_backup_until:
            cmd.angular.z = 0.0
            cmd.linear.x = self.WEDGE_BACKUP_V
        else:
            wedge_disp = self._check_progress(t_now)
            if (wedge_disp < self.WEDGE_MIN_DISP_M
                ...
                self.get_logger().warn(
                    ...
                    f'<{self.WEDGE_MIN_DISP_M}m -> reversing {self.WEDGE_BACKUP_V}m/s '
                    f'for {self.WEDGE_BACKUP_S}s')
                cmd.angular.z = 0.0
                cmd.linear.x = self.WEDGE_BACKUP_V
...
def main():
    ...
    ap.add_argument('--max-vel', type=float, default=0.8)
