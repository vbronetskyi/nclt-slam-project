...
class HybridGoalSender(Node):
    ...
    def _read_gt_pose(self):
        """Sim-only GT readout from /tmp/isaac_pose.txt for the precise
        finisher that lands the physical robot on the teach endpoint.
        """
        try:
    ...
    def _precise_approach(self, tx, ty, budget=60.0, tol=0.5):
        """Final endpoint finisher. Silence pure-pursuit via empty Path,
        then drive /cmd_vel directly using GT until within `tol` of target.
        """
        empty = Path()
        ...
        self.get_logger().info(
            f'  PRECISE APPROACH -> END ({tx:.2f},{ty:.2f})')
        t0 = time.time()
        stop = Twist()
        d = 999.0
        while time.time() - t0 < budget:
            ...
            if d < tol:
                for _ in range(3):
                    self.cmd_pub.publish(stop); time.sleep(0.05)
                self.get_logger().info(
                    f'  PRECISE ARRIVED END (d={d * 100:.1f} cm)')
                return True
        ...
        self.get_logger().warn(
            f'  PRECISE TIMEOUT END (d={d * 100:.0f} cm)')
        return False
    ...
    def run(self):
        ...
        for i in range(start, self.n_wps):
            ...
            # v64: precise finisher for the LAST WP only - close the
            # loop to < 0.5 m of the teach endpoint via direct GT control.
            if i == self.n_wps - 1:
                ex, ey = self.original_wps[-1]
                ok = self._precise_approach(ex, ey)
                if not ok:
                    ok = self._precise_approach(ex, ey, budget=45.0)
                self.reached += 1 if ok else 0
                if not ok:
                    self.skipped += 1
                continue
            if reached_ok:
        ...
        # v64 STOP: silence pp_follower + zero cmd_vel so the robot halts
        # cleanly. Exp 59 left pp chasing its last plan for minutes.
        empty = Path()
        empty.header.frame_id = 'map'
        empty.header.stamp = self.get_clock().now().to_msg()
        for _ in range(3):
