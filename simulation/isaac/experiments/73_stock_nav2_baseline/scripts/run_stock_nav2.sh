#!/bin/bash
# Stock Nav2 baseline run on route 09_se_ne.
#
...
ROUTE=09_se_ne
E73=/workspace/simulation/isaac/experiments/73_stock_nav2_baseline
...
OUT=${STOCK_OUT_DIR:-$E73/results/run_09}
mkdir -p $OUT
...
python3 - <<PY
import re
p='$E73/config/nav2_stock_params.yaml'
txt=open(p).read()
txt=re.sub(r'yaml_filename:.*', f'yaml_filename: "$TEACH_MAP"', txt, count=1)
open(p,'w').write(txt)
p='$E73/config/nav2_launch_stock.py'
txt=open(p).read()
...
pkill -9 -f "python.*run_husky_forest|rgbd_inertial|python.*tf_wall_clock|planner_server|map_server|controller_server|behavior_server|bt_navigator|python.*pure_pursuit_path|python.*send_goals|python.*nav_through_poses_client|lifecycle_manager|python.*costmap_snapshotter|python.*turnaround_supervisor|python.*plan_logger|python.*visual_landmark|ros2 launch" 2>/dev/null || true
sleep 5
rm -rf /dev/shm/fastrtps_* 2>/dev/null
rm -f /tmp/slam_pose.txt /tmp/slam_stop /tmp/slam_status.txt /tmp/isaac_pose.txt /tmp/isaac_imu.txt /tmp/isaac_clear_route.txt /tmp/isaac_remove_obstacles.txt /tmp/r73_*.sh
...
EOF
chmod +x /tmp/r73_tf_gt.sh
nohup bash /tmp/r73_tf_gt.sh > $OUT/tf_warmup.log 2>&1 &
...
echo "=== PHASE 2: tf_slam v55 + matcher + stock Nav2 + supervisor ==="
touch /tmp/isaac_clear_route.txt
sleep 5
kill $TF 2>/dev/null || true; sleep 2

cat > /tmp/r73_tf_slam.sh <<EOF
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=$DOMAIN
...
EOF
chmod +x /tmp/r73_tf_slam.sh
nohup bash /tmp/r73_tf_slam.sh > $OUT/tf_slam.log 2>&1 &
TF=$!; disown $TF; echo "TF(SLAM v55): $TF"
sleep 3

cat > /tmp/r73_matcher.sh <<EOF
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=$DOMAIN
...
EOF
chmod +x /tmp/r73_matcher.sh
nohup bash /tmp/r73_matcher.sh > $OUT/landmark_matcher.log 2>&1 &
MATCHER=$!; disown $MATCHER; echo "LandmarkMatcher: $MATCHER"

cat > /tmp/r73_nav2.sh <<EOF
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=$DOMAIN
export PYTHONPATH=/workspace/simulation/isaac/scripts:\${PYTHONPATH:-}
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec ros2 launch $E73/config/nav2_launch_stock.py
EOF
chmod +x /tmp/r73_nav2.sh
nohup bash /tmp/r73_nav2.sh > $OUT/nav2.log 2>&1 &
NAV2=$!; disown $NAV2; echo "Nav2(stock): $NAV2"

cat > /tmp/r73_sup.sh <<EOF
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=$DOMAIN
...
EOF
chmod +x /tmp/r73_sup.sh
nohup bash /tmp/r73_sup.sh > $OUT/supervisor.log 2>&1 &
...
echo "=== PHASE 3: nav_through_poses client (teach WP feeder) ==="
sleep 3
cat > /tmp/r73_client.sh <<EOF
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=$DOMAIN
export PYTHONPATH=/workspace/simulation/isaac/scripts:\${PYTHONPATH:-}
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec python3 $E73/scripts/waypoint_follower_client.py \
    --trajectory $TRAJ --spacing 4.0
EOF
chmod +x /tmp/r73_client.sh
nohup bash /tmp/r73_client.sh > $OUT/goals.log 2>&1 &
CLIENT=$!; disown $CLIENT; echo "Client: $CLIENT"

echo ""
echo "PIDs: Isaac=$ISAAC VIO=$VIO TF=$TF Matcher=$MATCHER Nav2=$NAV2 Sup=$SUP Client=$CLIENT"
echo "OUT: $OUT"
...
START_TS=$(date +%s)
cleanup_all() {
    pkill -TERM -f "visual_landmark_matcher|nav_through_poses_client" 2>/dev/null || true
    sleep 3
    pkill -9 -f "run_husky_forest|rgbd_inertial|tf_wall_clock|planner_server|map_server|controller_server|behavior_server|bt_navigator|pure_pursuit_path|turnaround_supervisor|visual_landmark_matcher|nav_through_poses_client|lifecycle_manager|ros2 launch" 2>/dev/null || true
    sleep 2
}
