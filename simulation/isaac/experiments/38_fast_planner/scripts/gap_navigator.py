...
class GapNavigator:
    ...
    def compute_cmd_vel_from_profile(self, obstacle_profile, desired_heading_error):
        """
        Same logic as compute_cmd_vel but uses a pre-computed obstacle profile
        (e.g. from LocalObstacleGrid) instead of raw depth image.
        The profile has fewer sectors (30-50) than the full 640-pixel depth.
        """
        ...
        kernel = np.ones(2 * half_s + 1, dtype=np.uint8).reshape(1, -1)
        inflated = cv2.erode(
            free_mask.astype(np.uint8).reshape(1, -1), kernel, iterations=1
        ).astype(bool).flatten()

        # Remap col_to_angle for this profile size
        angles = np.linspace(
        ...
        for col in range(n):
            if inflated[col] and not in_gap:
                ...
                g = self._make_gap_from_sectors(start, col - 1,
                                                obstacle_profile, angles)
                if g:
                    gaps.append(g)
                in_gap = False
        if in_gap:
            g = self._make_gap_from_sectors(start, n - 1,
                                            obstacle_profile, angles)
            if g:
                gaps.append(g)

        # Score with route constraint
        scored = self.score_gaps(gaps, desired_heading_error)
        self.prev_gaps.append(gaps)
        if len(self.prev_gaps) > self.HISTORY_SIZE:
            self.prev_gaps.pop(0)
        ...
        # Determine mode
        blocked_ratio = sum(1 for d in obstacle_profile
                            if d < self.OBSTACLE_THRESHOLD) / n
        c3 = n // 6
        center_min = float(np.min(
            obstacle_profile[n // 2 - c3: n // 2 + c3]
        )) if c3 > 0 else float(np.min(obstacle_profile))

        if not scored and not gaps:
            self.mode = "STUCK"
        elif not scored and gaps:
            self.mode = "STUCK"
        elif blocked_ratio > 0.5 or center_min < self.OBSTACLE_THRESHOLD:
            self.mode = "GAP_MODE"
        else:
            self.mode = "ROUTE_TRACKING"
        ...
        if self.mode == "ROUTE_TRACKING":
            ...
            if blocked_ratio < 0.1:
                ...
                lin = self.MAX_LINEAR * 0.7
        else:  # GAP_MODE
            ang = np.clip(selected["center_angle"] * 2.0,
                          -self.SLOW_SAFE_ANGULAR, self.SLOW_SAFE_ANGULAR)
            lin = self.SLOW_SAFE_LINEAR
            if abs(ang) > self.SLOW_SAFE_ANGULAR * 0.6:
                lin *= 0.5

        return float(lin), float(ang), self.mode, debug

    def _make_gap_from_sectors(self, left, right, profile, angles):
        """Create gap dict from sector indices."""
        center = (left + right) // 2
        ca = float(angles[center])
        aw = float(angles[right] - angles[left])
        if aw < 0.05:
            return None
        seg = profile[left: right + 1]
        valid = seg[(seg > self.MIN_VALID_DEPTH) & np.isfinite(seg)]
        cl = float(np.min(valid)) if len(valid) > 0 else self.MAX_RANGE
        md = float(np.median(valid)) if len(valid) > 0 else self.MAX_RANGE
        pw = 2.0 * md * math.tan(aw / 2.0)
        return {
            "left_col": left, "right_col": right, "center_col": center,
            "center_angle": ca, "angular_width": aw,
