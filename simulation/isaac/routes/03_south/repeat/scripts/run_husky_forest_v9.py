...
def _make_cam_matrix(x, y, z, yaw, pitch=0):
    # TODO ask supervisor if SE(3) or Sim(3) for our case
...
def _compute_synth_imu(pos_arg, quat_xyzw, pos_for_vel, t):
    ...
    # Angular velocity from quaternion diff (body frame)
    ...
    # Standstill detection on full 100ms window of GT positions
    _synth_pos_hist.append(pos.copy())
    is_stationary = False
    ...
    if is_stationary:
        # Stationary: override accel with pure gravity + Phidgets noise.
        # Buffer is still fed so resumption starts from realistic state.
