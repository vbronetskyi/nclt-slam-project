...
#
# Consumes:
#   /workspace/.../71_teach_05_ne_sw_accel_noise/teach/05_ne_sw/
...
E72=/workspace/simulation/isaac/routes/05_ne_sw/repeat
SCRIPTS=/workspace/simulation/isaac/scripts
SLAM=/workspace/third_party/ORB_SLAM3
DOMAIN=${ROS_DOMAIN_ID:-85}

TEACH=/root/isaac_tr_datasets/05_ne_sw/teach/teach_outputs
...
# ensure /tmp/slam_routes.json has 05_ne_sw
python3 /workspace/simulation/isaac/routes/_common/scripts/register_routes.py

#--obstacles enables spawn_obstacles["05_ne_sw"] - 3 cone groups + tent
setsid /opt/isaac-sim-6.0.0/python.sh /workspace/simulation/isaac/scripts/common/run_husky_forest.py \
    --synthetic-imu --route 05_ne_sw --obstacles --duration 4500 \
    --spawn-x 65.00 --spawn-y 35.00 --spawn-yaw -2.6204 \
