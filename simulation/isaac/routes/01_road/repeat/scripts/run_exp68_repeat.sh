...
E66=/workspace/simulation/isaac/routes/01_road/teach
E68=/workspace/simulation/isaac/routes/01_road/repeat
...
OUT=${REPEAT_OUT_DIR:-$E68/results/repeat_run}
mkdir -p $OUT/plans
...
import re
for p, patterns in [
    ('$E68/config/nav2_planner_only.yaml', [(r'yaml_filename:.*', f'yaml_filename: "$TEACH_MAP"')]),
    ('$E68/config/nav2_launch_hybrid.py',  [
        (r'config = "[^"]*"',   f'config = "$E68/config/nav2_planner_only.yaml"'),
...
rm -f /tmp/slam_pose.txt /tmp/slam_stop /tmp/slam_status.txt /tmp/isaac_pose.txt /tmp/isaac_imu.txt /tmp/isaac_clear_route.txt /tmp/isaac_remove_obstacles.txt /tmp/exp68r_*.sh
...
setsid ./Examples/RGB-D-Inertial/rgbd_inertial_live \
    Vocabulary/ORBvoc.txt $E68/config/vio_th160.yaml $REC \
    </dev/null > $OUT/vio.log 2>&1 &
VIO=$!; disown $VIO; echo "VIO: $VIO"

cat > /tmp/exp68r_tf_gt.sh <<EOF
#!/bin/bash
...
EOF
chmod +x /tmp/exp68r_tf_gt.sh
nohup bash /tmp/exp68r_tf_gt.sh > $OUT/tf_warmup.log 2>&1 &
...
export ROS_DOMAIN_ID=$DOMAIN
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec python3 $E68/scripts/tf_wall_clock_relay_v55.py --slam-encoder
EOF
chmod +x /tmp/exp68r_tf_slam.sh
nohup bash /tmp/exp68r_tf_slam.sh > $OUT/tf_slam.log 2>&1 &
TF=$!; disown $TF; echo "TF(SLAM v55): $TF"
sleep 3

cat > /tmp/exp68r_matcher.sh <<EOF
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=$DOMAIN
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec python3 $E68/scripts/visual_landmark_matcher.py \
    --landmarks $LANDMARKS --out-csv $OUT/anchor_matches.csv
EOF
chmod +x /tmp/exp68r_matcher.sh
nohup bash /tmp/exp68r_matcher.sh > $OUT/landmark_matcher.log 2>&1 &
MATCHER=$!; disown $MATCHER; echo "LandmarkMatcher: $MATCHER"

cat > /tmp/exp68r_nav2.sh <<EOF
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=$DOMAIN
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec ros2 launch $E68/config/nav2_launch_hybrid.py
EOF
chmod +x /tmp/exp68r_nav2.sh
nohup bash /tmp/exp68r_nav2.sh > $OUT/nav2.log 2>&1 &
NAV2=$!; disown $NAV2; echo "Nav2: $NAV2"

cat > /tmp/exp68r_pp.sh <<EOF
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=$DOMAIN
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec python3 $E68/scripts/pure_pursuit_path_follower.py
EOF
chmod +x /tmp/exp68r_pp.sh
nohup bash /tmp/exp68r_pp.sh > $OUT/pp_follower.log 2>&1 &
PP=$!; disown $PP; echo "PP: $PP"

cat > /tmp/exp68r_sup.sh <<EOF
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=$DOMAIN
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec python3 $E68/scripts/turnaround_supervisor.py --turnaround-x 70.0 --past-margin 2.0
EOF
chmod +x /tmp/exp68r_sup.sh
nohup bash /tmp/exp68r_sup.sh > $OUT/supervisor.log 2>&1 &
SUP=$!; disown $SUP; echo "Supervisor: $SUP"

cat > /tmp/exp68r_plan.sh <<EOF
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=$DOMAIN
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec python3 $E68/scripts/plan_logger.py --out-dir $OUT/plans
EOF
chmod +x /tmp/exp68r_plan.sh
nohup bash /tmp/exp68r_plan.sh > $OUT/plan_logger.log 2>&1 &
...
sleep 3
cat > /tmp/exp68r_goals.sh <<EOF
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=$DOMAIN
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec python3 $E68/scripts/send_goals_hybrid.py \
    --trajectory $TRAJ \
    --spacing 4.0 --goal-timeout 300 --tolerance 3.0
EOF
chmod +x /tmp/exp68r_goals.sh
nohup bash /tmp/exp68r_goals.sh > $OUT/goals.log 2>&1 &
