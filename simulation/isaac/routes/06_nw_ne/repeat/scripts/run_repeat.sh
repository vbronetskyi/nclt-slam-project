...
#
# Consumes:
#/workspace/.../71_teach_06_nw_ne_accel_noise/teach/06_nw_ne/
...
E72=/workspace/simulation/isaac/routes/06_nw_ne/repeat
SCRIPTS=/workspace/simulation/isaac/scripts
SLAM=/workspace/third_party/ORB_SLAM3
DOMAIN=${ROS_DOMAIN_ID:-85}

TEACH=/root/isaac_tr_datasets/06_nw_ne/teach/teach_outputs
...
# ensure /tmp/slam_routes.json has 06_nw_ne
python3 /workspace/simulation/isaac/routes/_common/scripts/register_routes.py

#--obstacles enables spawn_obstacles["06_nw_ne"] - 3 cone groups + tent
setsid /opt/isaac-sim-6.0.0/python.sh /workspace/simulation/isaac/scripts/common/run_husky_forest.py \
    --synthetic-imu --route 06_nw_ne --obstacles --duration 4500 \
    --spawn-x -90.00 --spawn-y 35.00 --spawn-yaw 0.0236 \
...
export PYTHONPATH=/workspace/simulation/isaac/scripts:\${PYTHONPATH:-}
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec python3 /workspace/simulation/isaac/scripts/common/turnaround_supervisor.py --final-x 65.0 --final-y 35.0 --near-radius 10.0
EOF
