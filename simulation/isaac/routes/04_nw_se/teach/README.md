...
- total waypoints: **482** (241 outbound + turnaround arc + mirrored return)
- total path length (incl. return): **384.9 m**
...
1. `register_routes.py` merges `_common/routes_04_07.json` into
   ...
   `--route 04_nw_se` as a predefined path.
2. Isaac Sim launches with:
   `--synthetic-imu --route 04_nw_se --duration 4500`
...
## outputs (teach/04_nw_se/ after a successful run)
