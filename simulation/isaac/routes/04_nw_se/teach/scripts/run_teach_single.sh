...
E66=/workspace/simulation/isaac/routes/04_nw_se/teach
...
if [ "$ROUTE" != "04_nw_se" ]; then
    echo "this teach script is dedicated to route 04_nw_se (got: $ROUTE)" >&2
    exit 1
fi
...
EOF
chmod +x /tmp/04_tf.sh
nohup bash /tmp/04_tf.sh > $OUT/tf_slam.log 2>&1 &
...
EOF
chmod +x /tmp/04_drift.sh
nohup bash /tmp/04_drift.sh > $OUT/drift_monitor.log 2>&1 &
DRIFT=$!; disown $DRIFT; echo "DriftMonitor: $DRIFT"

cat > /tmp/04_depth.sh <<EOF
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=$DOMAIN
...
EOF
chmod +x /tmp/04_depth.sh
nohup bash /tmp/04_depth.sh > $OUT/teach_depth_mapper.log 2>&1 &
DEPTH=$!; disown $DEPTH; echo "DepthMapper: $DEPTH"

cat > /tmp/04_lm.sh <<EOF
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=$DOMAIN
...
EOF
chmod +x /tmp/04_lm.sh
nohup bash /tmp/04_lm.sh > $OUT/landmark_recorder.log 2>&1 &
