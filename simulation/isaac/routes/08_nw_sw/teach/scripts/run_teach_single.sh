...
if [ "$ROUTE" != "08_nw_sw" ]; then
    echo "this teach script is dedicated to route 08_nw_sw (got: $ROUTE)" >&2
    exit 1
fi
ORIG_X=-110; ORIG_Y=-45; W=195; H=90
SPAWN_X=-90.00; SPAWN_Y=35.00; SPAWN_YAW=-2.0921
...
EOF
chmod +x /tmp/08_tf.sh
nohup bash /tmp/08_tf.sh > $OUT/tf_slam.log 2>&1 &
...
EOF
chmod +x /tmp/08_drift.sh
nohup bash /tmp/08_drift.sh > $OUT/drift_monitor.log 2>&1 &
DRIFT=$!; disown $DRIFT; echo "DriftMonitor: $DRIFT"

cat > /tmp/08_depth.sh <<EOF
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=$DOMAIN
...
EOF
chmod +x /tmp/08_depth.sh
nohup bash /tmp/08_depth.sh > $OUT/teach_depth_mapper.log 2>&1 &
DEPTH=$!; disown $DEPTH; echo "DepthMapper: $DEPTH"

cat > /tmp/08_lm.sh <<EOF
#!/bin/bash
source /opt/ros/jazzy/setup.bash
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
export ROS_DOMAIN_ID=$DOMAIN
...
EOF
chmod +x /tmp/08_lm.sh
nohup bash /tmp/08_lm.sh > $OUT/landmark_recorder.log 2>&1 &
