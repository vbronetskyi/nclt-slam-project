...
plus a metadata YAML containing timestamp + origin + resolution + robot
pose at snapshot time.

Also writes a summary CSV: snapshot_id, wall_ts, robot_x, robot_y,
occ_cells, unknown_cells.

Pair with post-run analysis to animate the costmap evolution as robot
passes cones, observing:
  - teach map (static layer) frozen content
  - obstacle_layer marks/clears as robot approaches/leaves cones
  - NavFn path replanning in response
"""
import argparse
import math
import os
