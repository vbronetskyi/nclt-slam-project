...
E66=/workspace/simulation/isaac/routes/07_se_sw/teach
E72=/workspace/simulation/isaac/routes/07_se_sw/repeat
SCRIPTS=/workspace/simulation/isaac/scripts
SLAM=/workspace/third_party/ORB_SLAM3
DOMAIN=${ROS_DOMAIN_ID:-85}

TEACH=/root/isaac_tr_datasets/07_se_sw/teach/teach_outputs
...
# ensure /tmp/slam_routes.json has 07_se_sw
python3 /workspace/simulation/isaac/routes/_common/scripts/register_routes.py

# --obstacles enables spawn_obstacles["07_se_sw"] - 3 cone groups + tent
setsid /opt/isaac-sim-6.0.0/python.sh $E72/scripts/run_husky_forest.py \
    --synthetic-imu --route 07_se_sw --obstacles --duration 4500 \
    --spawn-x 65.00 --spawn-y -35.00 --spawn-yaw 3.1416 \
...
export PYTHONPATH=/workspace/simulation/isaac/scripts:\${PYTHONPATH:-}
export LD_LIBRARY_PATH=\${LD_LIBRARY_PATH:-}:/opt/isaac-sim-6.0.0/exts/isaacsim.ros2.core/jazzy/lib
exec python3 $E72/scripts/turnaround_supervisor.py --final-x -90.0 --final-y -35.0 --near-radius 10.0
EOF
