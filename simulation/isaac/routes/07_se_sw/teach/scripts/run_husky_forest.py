...
parser.add_argument("--route", type=str, default=None,
                    choices=["road", "north", "south", "road_nav2", "warmup", "north_forest", "07_se_sw"],
                    help="predefined route: road (S-curve), north (forest), south (forest), warmup (straight), north_forest (deep N-forest teach, exp 71), 07_se_sw (corner-to-corner teach)")
...
_husky_translate_op.Set(Gf.Vec3d(_spawn_x, _spawn_y, 10.0))  # high placeholder - updated after terrain_height defined. Must stay ABOVE any terrain peak (+-1.0 m) to avoid PhysX spawning robot inside the mesh during warmup ticks.
...
elif args.route == "07_se_sw":
    import json as _json
    with open(_ROUTES_FILE) as _rf:
        _all_routes = _json.load(_rf)
    _auto_route = list(_all_routes.get("07_se_sw", []))
    print(f"  AUTO ROUTE: 07_se_sw ({len(_auto_route)} waypoints)")
