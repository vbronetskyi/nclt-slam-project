...
*[thesis root](../../../../../README.md) > [simulation](../../../../README.md) > isaac > routes > 07_se_sw > teach*


Corner-to-corner roundtrip teach: **bottom edge RB->LB**
Route: (+65, -35) -> (-90, -35), smooth hairpin turnaround (r=1.5 m, 180° arc + parallel-offset
blended return of 10 pts), mirror outbound for the return leg.

## Numbers (from `_common/routes_04_07.json`)
- total waypoints: **502** (251 outbound + turnaround arc + mirrored return)
- total path length (incl. return): **400.9 m**
- spawn: x=+65.00, y=-35.00, yaw=+3.142 rad (+180°)
- min clearance from any scene-object edge: **≥ 1.9 m** (robot r=0.4 m, so
  physical body-to-obstacle gap ≥ 1.5 m)
...
1. `register_routes.py` merges `_common/routes_04_07.json` into
   `/tmp/slam_routes.json` so `run_husky_forest.py` can pick up
   `--route 07_se_sw` as a predefined path.
2. Isaac Sim launches with:
   `--synthetic-imu --route 07_se_sw --duration 4500`
...
## Outputs (teach/07_se_sw/ after a successful run)
