#!/usr/bin/env python3
"""tf + odom relay for Nav2 <- isaac sim, the central piece of our T&R stack

reads pose from /tmp/isaac_pose.txt (written by run_husky_forest), fuses
it with encoder or IMU, snaps to anchor corrections from
/anchor_correction when those are available, and pushes out the
map->base_link TF, an /odom topic plus the static base_link -> camera_link
and base_link -> imu_link TFs

usage:
  source /opt/ros/jazzy/setup.bash
  python3 tf_wall_clock_relay_v55.py --slam-encoder
"""
import math
import re
import sys
...
SPAWN_X, SPAWN_Y = -95.0, -6.0  # south route default; override via --spawn-x/y
#this default is for 03_south. routes 04-09 override from run_repeat.sh.   

# fusion parameters.  exp 59 values, stable since
#SLAM dominant for translation because encoder drifts on skid-steer
# yaw mostly from encoder/gyro since SLAM yaw is noisier than integrated wheel yaw   
SLAM_POS_ALPHA = 0.7   # exp 55 was 0.5, underweighted SLAM and drift built up fast
ODOM_POS_ALPHA = 0.3   # paired with SLAM_POS_ALPHA (sum = 1.0)
SLAM_YAW_ALPHA = 0.15  # exp 58 was 0.3,  amplified SLAM yaw noise
GYRO_DEADZONE = 0.08   # below this rad/s the gyro is pure noise on a stopped robot
GYRO_LPF_ALPHA = 0.3   # lowpass on the remaining gyro signal
JUMP_THRESHOLD = 0.5   # metres, pose delta > this between ticks = reject or snap
YAW_JUMP_THRESHOLD = 0.3  # rad,  same idea for heading

# dropped exp 58: running landmark accumulator.  idea was to collect new
# landmarks at 5s/5m intervals during repeat to handle route changes.  in
# practice this was a drift REINFORCEMENT loop - new landmarks registered on
# top of the VIO drift, so each new anchor pulled pose further off.  see
# CHANGELOG 0.58
# ACCUMULATOR_ENABLED = True
# ACCUMULATOR_MIN_GAP_S = 5.0
# ACCUMULATOR_MIN_DIST_M = 5.0

# dropped exp 61: dense anchor mode (anchor every 2s or 2m).  also a drift
# loop, when anchors fire too often they self-consistent around current
# drifted pose
# ANCHOR_DENSE_MODE = False  # kept flag, never re-enabled
# ANCHOR_DENSE_PERIOD_S = 2.0
...
class TFRelay(Node):
    def __init__(self, use_gt=False, slam_frame=False, encoder_imu=False, slam_encoder=False):
        ...
        # numbers from the husky URDF + d435i + phidgets mount
        # if you change the mesh offsets in the URDF, change them here too
        # yes duplicate source of truth, yes should be read from
        # robot_description.  one day
        ...
        cam_tf.transform.translation.x = 0.5   # camera is 0.5 m forward from base
        cam_tf.transform.translation.z = 0.48  # 0.48 m up
        ...
        # v55 visual-anchor correction subscriber
        # matcher publishes /anchor_correction when teach-map features match
        # see visual_landmark_matcher.py for how that message gets built
        from geometry_msgs.msg import PoseWithCovarianceStamped
        self.anchor_last = None         # (ts, x, y, z, qx, qy, qz, qw, std)
        self.anchor_strong_streak = 0   # consecutive strong matches, for hysteresis

        # anchor tresholds - these three numbers dominate behaviour, tune with care
        self.ANCHOR_STALE_S = 3.0       # anchor older than this = ignored
        self.ANCHOR_STRONG_STD = 0.1    # std below this = strong
        self.ANCHOR_OK_STD = 0.2        # std between strong and OK - weaker blend
        # exp 58 had ANCHOR_OK_STD=0.3 and still trusted anchors at std=0.28
        # got drift runaway because one noisy anchor locked pose away from GT
        # 0.2 is safer
        self.ANCHOR_HYSTERESIS_N = 2    # need 2 consecutive strong matches before "strong" regime

        # dropped exp 62 variant: anchor sanity gate (seperate rejection pre-filter)
        # rejected anchors that agreed with encoder (enc_disagree < 0.5m) because
        # we assumed they were redundant.  turned out encoder can agree with anchor
        # even when both are drifted together, so the gate was catching nothing
        # self.ANCHOR_SANITY_ENC_M = 2.0
        # self.ANCHOR_SANITY_SHIFT_M = 0.5
        # self.ANCHOR_SANITY_STD = 0.15

        self.create_subscription(PoseWithCovarianceStamped,
                                 '/anchor_correction', self._anchor_cb, 10)
    ...
    def _anchor_cb(self, msg):
        """Receive visual anchor correction from visual_landmark_matcher.

        Stores (ts, pose, std) - the blend logic in _tick_slam_encoder
        applies hysteresis and weight choice.
        """
        std = math.sqrt(max(msg.pose.covariance[0], 1e-8))
        self.anchor_last = (
            pytime.time(),
        ...
        if std <= self.ANCHOR_STRONG_STD:
            self.anchor_strong_streak += 1
        else:
            self.anchor_strong_streak = max(0, self.anchor_strong_streak - 1)
    ...
    def _tick_slam_encoder(self, now, x, y, z, qx, qy, qz, qw):
        ...
        if slam_ok:
            # v55 regime switching:
            #   no_anchor  -> 95% SLAM + 5% encoder (baseline, same as v53)
            #   ok         -> 20% anchor + 75% SLAM + 5% encoder
            ...
            anchor_x_use = None
            anchor_y_use = None
            if self.anchor_last is not None:
                ats = self.anchor_last[0]
                staleness = pytime.time() - ats
                std = self.anchor_last[8]
                if staleness <= self.ANCHOR_STALE_S and std <= self.ANCHOR_OK_STD:
                    anchor_x_use = self.anchor_last[1]
                    anchor_y_use = self.anchor_last[2]
                    if (std <= self.ANCHOR_STRONG_STD
                            and self.anchor_strong_streak >= self.ANCHOR_HYSTERESIS_N):
                        regime = 'strong'
                    else:
                        regime = 'ok'
            if regime == 'strong':
                nav_x = 0.40 * anchor_x_use + 0.55 * slam_nx + 0.05 * self.enc_x
                nav_y = 0.40 * anchor_y_use + 0.55 * slam_ny + 0.05 * self.enc_y
            elif regime == 'ok':
                nav_x = 0.20 * anchor_x_use + 0.75 * slam_nx + 0.05 * self.enc_x
                nav_y = 0.20 * anchor_y_use + 0.75 * slam_ny + 0.05 * self.enc_y
            else:
                ...
                if anchor_age > 10.0:
                    if slam_enc_d < 2.0:
                        ...
                        alpha = 0.70
                    elif slam_enc_d < 10.0:
                        alpha = 0.40
                    else:
                        alpha = 0.10
                else:
                    alpha = 0.95
                nav_x = alpha * slam_nx + (1 - alpha) * self.enc_x
                nav_y = alpha * slam_ny + (1 - alpha) * self.enc_y
                self._slam_enc_d_noanchor = slam_enc_d
                self._alpha_noanchor = alpha
            nav_yaw = self.enc_yaw
            self.using_slam = True
            self._last_regime = regime
            self._last_anchor_staleness = (
                pytime.time() - self.anchor_last[0]) if self.anchor_last else -1.0
            self._last_anchor_std = (
                self.anchor_last[8]) if self.anchor_last else -1.0
        else:
        ...
        if self.log_counter % 100 == 0:
            ...
            regime = getattr(self, '_last_regime', 'no_anchor')
            anchor_age = getattr(self, '_last_anchor_staleness', -1.0)
            anchor_std = getattr(self, '_last_anchor_std', -1.0)
            self.get_logger().info(
                ...
                f'regime={regime} anchor_age={anchor_age:.1f}s std={anchor_std:.2f}')
    ...
    def _tick_slam_frame(self, now, x, y, z, qx, qy, qz, qw):
        ...
        # SLAM jump detection.  when SLAM loses and re-acquires tracking the
        # pose can snap several metres at once,  thats a "jump"
        # trusting it blindly corrupts the fused pose, so we reject jumps
        # and drift on encoder for a tick
        # this happens +-1-2 times per route in practice
        ...
        if slam_jump < JUMP_THRESHOLD:
            ...
            # jump, trust encoder for this tick
            self.fused_x = predicted_x
            self.fused_y = predicted_y
            self.get_logger().warn(f'SLAM jump {slam_jump:.1f}m rejected')

        # dropped exp 60: progressive blend weight based on jump magnitude
        # was trying a smooth transition from SLAM-trust to encoder-trust based
        # on jump size, but it made behaviour harder to reason about when
        # debugging  hard cutoff at JUMP_THRESHOLD is cleaner
        # jump_weight = 1.0 - min(1.0, slam_jump / JUMP_THRESHOLD)
        # self.fused_x = jump_weight * (SLAM_POS_ALPHA * nav_x + ODOM_POS_ALPHA * predicted_x) + (1-jump_weight) * predicted_x
