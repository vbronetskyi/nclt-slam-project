...
# matching knobs
MIN_MATCHES = 10                      # raw match count gate before RANSAC
LOWE_RATIO = 0.80                     # not used with crossCheck matcher, kept for docs
REPROJ_MAX_PX = 2.0                   # median reprojection error after PnP
RANSAC_REPROJ_PX = 3.0                # ransac inlier treshold
RANSAC_ITERATIONS = 200
MIN_INLIERS = 10                      # gate on RANSAC inlier count
...
# landmark > ACCUM_MIN_DIST_M away, record current frame's ORB features as a
# new landmark with current VIO pose.  ran once, saw 83 m peak drift, reverted
ACCUM_ENABLE = True
ACCUM_SILENCE_S = 5.0
ACCUM_MIN_DIST_M = 5.0
ACCUM_MIN_KPTS = 30
ACCUM_SAVE_PKL = True

# dropped exp 57: depth-correlation matcher.  the idea was to cross-correlate
...
#         sub_cur = _grid_avg(cur_depth, grid)
#         sub_td = _grid_avg(td, grid)
...
class VisualLandmarkMatcher(Node):
    ...
    def _tick(self):
        ...
        for li in cand_idx:
            ...
            # whole pipeline, most false matches get caught here
