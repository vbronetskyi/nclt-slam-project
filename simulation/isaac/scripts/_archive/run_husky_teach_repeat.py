...
if not args.use_gt:
    ...
    localizer = None
    print("  GT MODE - using ground truth for localization")
...
mode = "GT" if args.use_gt else "VISUAL"
print(f"\n{'='*60}")
print(f"TEACH-AND-REPEAT: route={args.route} direction={args.direction} mode={mode}")
...
try:
    while sim_time < args.duration and not follower.finished:
        ...
        # 2. Localize
        anchor_id = follower.current_anchor_idx
        confidence = 1.0

        if args.use_gt:
            # GT mode: find nearest anchor to GT position
            if frame_count % LOCALIZE_EVERY == 0:
                gx, gy, _, gyaw = _get_husky_pose()
                best_i = 0
                best_d = float("inf")
                for i, a in enumerate(nav_anchors):
                    d = math.hypot(gx - a["x"], gy - a["y"])
                    if d < best_d:
                        best_d = d
                        best_i = i
                follower.odom_x = gx
                follower.odom_y = gy
                follower.odom_yaw = gyaw
                follower.current_anchor_idx = best_i
                anchor_id = best_i
                confidence = 1.0
        else:
            ...
            if frame_count % LOCALIZE_EVERY == 0:
                try:
                    ...
                    if d_fwd is not None and d_fwd.shape[0] > 10:
                        import cv2
                        gray = cv2.cvtColor(d_fwd[:, :, :3].copy(),
                                            cv2.COLOR_RGB2GRAY)
