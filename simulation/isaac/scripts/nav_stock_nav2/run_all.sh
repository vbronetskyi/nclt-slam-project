...
EXP_DIR=/workspace/simulation/isaac/experiments/74_pure_stock_nav2_baseline
...
SUMMARY_FILE=${SUMMARY_FILE:-/tmp/run_all_baseline_74_summary.txt}
: > "$SUMMARY_FILE"
...
on_interrupt() {
    echo ""
    echo "!!! exp 74 orchestrator received signal - cleanup"
...
for R in "${ROUTES[@]}"; do
    ...
    echo ""
    echo "=== EXP 74 / $R  $STATUS  ${DUR}s  at $(date +'%T') ==="
