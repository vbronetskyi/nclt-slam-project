"""Tests for sensor loaders, synchronizer, session manager, and utility functions."""
...
@pytest.fixture
def data_dir() -> Path:
    """Return the path to the mock session data directory."""
...
def sensor_config() -> dict:
    """Return a copy of the sensor configuration dictionary."""
    return dict(SENSOR_CONFIG)


@pytest.fixture
def imu_loader(data_dir: Path):
    """Return a fresh IMULoader pointed at mock ms25.csv."""
...
def odometry_loader(data_dir: Path):
    """Return a fresh OdometryLoader pointed at mock odometry."""
...
def kvh_loader(data_dir: Path):
    """Return a fresh KVHLoader pointed at mock kvh.csv."""
...
def groundtruth_loader(data_dir: Path):
    """Return a fresh GroundTruthLoader pointed at mock groundtruth.csv."""
...
def session_manager(data_dir: Path, sensor_config: dict):
    """Return a SessionSensorManager for the mock session."""
...
class TestBaseSensorLoader:
    """Tests for BaseSensorLoader using IMULoader on mock ms25.csv."""

    def test_load(self, imu_loader):
        """Load data and verify shape and column names."""
    ...
    def test_query_exact(self, imu_loader):
        """Query an exact timestamp and verify a non-None result."""
    ...
    def test_query_within_window(self, imu_loader):
        """Query a timestamp offset by 5000 us; should still find a match."""
    ...
    def test_query_outside_window(self, imu_loader):
        """Query a timestamp 100 ms outside the data range; should be None."""
        data = imu_loader.load()
    ...
    def test_slice(self, imu_loader):
        """Slice a 1-second window and verify ~100 samples at 100 Hz."""
        start = T0
    ...
    def test_interpolate(self, imu_loader):
        """Interpolate between two timestamps and verify the result."""
        ...
        # The interpolated value should lie between the two bounding rows
        data = imu_loader.load()
        v0 = data.values[0]
        v1 = data.values[1]
    ...
    def test_interpolate_outside_range(self, imu_loader):
        """Interpolation outside the recorded range should return None."""
...
class TestIndividualLoaders:
    """Tests verifying column counts and names for each concrete loader."""

    def test_imu_loader(self, imu_loader):
        """IMU loader: 9 value columns with correct names."""
        data = imu_loader.load()
    ...
    def test_gps_loader(self, gps_loader):
        """GPS loader: 7 value columns with correct names."""
    ...
    def test_groundtruth_loader(self, groundtruth_loader):
        """Ground truth loader: 6 value columns and valid SE(3) pose matrix."""
